#pragma once
#ifndef INCLUDE_UTIL_
#define INCLUDE_UTIL_
...
bool str_a_contain_str_b(const std::string &a, const std::string &b)
{
...
void Tab_to_qab_tab(Eigen::Matrix4d Tab, Eigen::Quaterniond &qab, Eigen::Vector3d &tab)
{
    Eigen::Matrix3d Rab = Tab.block(0, 0, 3, 3);
    qab = Eigen::Quaterniond(Rab);
    tab = Tab.block(0, 3, 3, 1);
}

Eigen::Vector3d Tab_pbc_to_tac(Eigen::Matrix4d Tab, Eigen::Vector3d &pbc)
{
    Eigen::Matrix3d Rab = Tab.block(0, 0, 3, 3);
    Eigen::Vector3d tab = Tab.block(0, 3, 3, 1);
    return Rab * pbc + tab;
}

Eigen::Matrix3d fxycxy_to_K(Eigen::Vector4d fxycxy)
{
...
static inline Eigen::Matrix4d Tab_to_Tba(const Eigen::Matrix4d &Tab)
{
    Eigen::Matrix4d Tba = Eigen::Matrix4d::Identity();
    Eigen::Matrix3d Rba = Tab.block(0, 0, 3, 3).transpose();
    Eigen::Vector3d tba = -Rba * Tab.block(0, 3, 3, 1);
    Tba.block(0, 0, 3, 3) = Rba;
    Tba.block(0, 3, 3, 1) = tba;
    return Tba;
}
...
int utm2wgs(int zone, Eigen::Vector3d &utm, Eigen::Vector3d &wgs)
{
    wgs = utm;
    std::string str = "+proj=utm +zone=" + std::to_string(zone) + "N +ellps=WGS84 +units=m +no_defs";
