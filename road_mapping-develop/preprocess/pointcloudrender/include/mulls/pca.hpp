//
// This file is used for the  Principle Component Analysis (PCA) and related feature calculation of
// Point Cloud. Dependent 3rd Libs: PCL (>1.7) By Yue Pan et al.
    ...
    public:
                             ...
                             pcl::PointCloud<pcl::Normal>::Ptr &normals)
        {
            ...
            ne.setNumberOfThreads(
                omp_get_max_threads()); // More threads sometimes would not speed up the procedure
...
#pragma omp parallel for // Multi-thread
        ...
        bool get_pc_pca_feature(typename pcl::PointCloud<PointT>::Ptr in_cloud,
                                std::vector<pca_feature_t> &features, float radius, int nearest_k,
                                int min_k = 1, int pca_down_rate = 1, bool distance_adaptive_on = false,
                                float unit_dist = 35.0)
        {
            ...
            std::chrono::duration<double> time_build_tree =
...
#pragma omp parallel for // Multi-thread
            for (int i = 0; i < in_cloud->points.size(); i += pca_down_rate)
            {
                ...
                std::vector<int> search_indices_used; // points would be stored in sequence (from the closest
                                                      // point to the farthest point within the neighborhood)
                std::vector<int> search_indices;      // point index Vector
                ...
                std::chrono::duration<double> time_neighbor_search =
                    std::chrono::duration_cast<std::chrono::duration<double>>(toc_1 - tic_1);
                ...
                std::chrono::duration<double> time_pca =
        ...
        bool get_pc_pca_feature(typename pcl::PointCloud<PointT>::Ptr in_cloud,
                                ...
                                typename pcl::KdTreeFLANN<PointT>::Ptr &tree, float radius, int nearest_k,
                                int min_k = 1, int pca_down_rate = 1, bool distance_adaptive_on = false,
                                float unit_dist = 35.0)
        {
            // LOG(INFO) << "[" << in_cloud->points.size() << "] points used for PCA, pca down rate is [" <<
            // pca_down_rate << "]";
...
#pragma omp parallel for                                                     // Multi-thread
                                            ...
                                            std::vector<pca_feature_t> &features)
        {
            for (int i = 0; i < in_cloud->points.size(); i++)
            {
                ...
                n_self << in_cloud->points[i].normal_x, in_cloud->points[i].normal_y,
                    in_cloud->points[i].normal_z;

                features[i].normal_diff_ang_deg =
                    180.0 / M_PI *
                    std::acos(std::abs(
                        n_mean.dot(n_self))); // n1.norm()=n2.norm()=1
                                              // if (i % 10 == 0)
                             ...
                             std::vector<int> &search_indices, pca_feature_t &feature)
        {
            ...
            if ((feature.values.lamada1 + feature.values.lamada2 + feature.values.lamada3) == 0)
                ...
                feature.curvature =
                    ...
                    (feature.values.lamada1 + feature.values.lamada2 + feature.values.lamada3);

            // feature.linear_2 = (sqrt(feature.values.lamada1) - sqrt(feature.values.lamada2)) /
            // sqrt(feature.values.lamada1); feature.planar_2 = (sqrt(feature.values.lamada2) -
            // sqrt(feature.values.lamada3)) / sqrt(feature.values.lamada1); feature.spherical_2 =
            // sqrt(feature.values.lamada3) / sqrt(feature.values.lamada1);
            feature.linear_2 =
                ((feature.values.lamada1) - (feature.values.lamada2)) / (feature.values.lamada1);
            feature.planar_2 =
                ((feature.values.lamada2) - (feature.values.lamada3)) / (feature.values.lamada1);
        ...
        // is_palne_feature (true: assign point normal as pca normal vector, false: assign point normal as
        // pca primary direction vector)
