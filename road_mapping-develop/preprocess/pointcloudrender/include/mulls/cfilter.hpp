//
// This file is used for the filtering and feature point extraction of Point
// Cloud. Dependent 3rd Libs: PCL (>1.7) By Yue Pan
//
...
namespace lo
{
    ...
    public:
        ...
        struct simplified_voxel_t
        {
            ...
            bool has_keypoint;
            simplified_voxel_t() { has_keypoint = false; }
        };

        bool voxel_downsample(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                              ...
                              float voxel_size)
        {
            ...
            if (voxel_size < 0.01)
            {
                // LI_INFO("too small voxel size, the downsampling would be disabled");
                cloud_out = cloud_in;
                return false;
            }
...
#pragma omp parallel for private(i) // Multi-thread
        ...
        bool normal_downsample(const typename pcl::PointCloud<PointT>::Ptr &cloud_in_out,
                               std::vector<int> &out_indices, int downsample_ratio, int bin_num = 3,
                               int rand_seed = 0)

        {
            ...
            std::chrono::steady_clock::time_point toc = std::chrono::steady_clock::now();
            std::chrono::duration<double> time_set_normal_used =
                std::chrono::duration_cast<std::chrono::duration<double>>(tic_2 - tic);
            std::chrono::duration<double> time_used =
                std::chrono::duration_cast<std::chrono::duration<double>>(toc - tic);

            // LOG(INFO) << "Normal space downsampling [ " << original_pts_num << " --> " <<
            // out_indices.size()
            //           << " ] done in [" << time_used.count() * 1000.0 << "] ms ["
            //           << time_set_normal_used.count() * 1000.0 << "] ms for setting the normal\n";
            return 1;
        }

        // SOR (Statisics Outliers Remover);
        bool sor_filter(typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                        typename pcl::PointCloud<PointT>::Ptr &cloud_out,
                        int mean_k,
                        double n_std)
        {
        ...
        // SOR (Statisics Outliers Remover)
        bool sor_filter(typename pcl::PointCloud<PointT>::Ptr &cloud_in_out,
                        int mean_k,
                        double n_std)
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
        ...
        // regenerate the calibrated point cloud (the intrinsic angle might be
        // uncorrect)
        bool vertical_intrinsic_calibration(typename pcl::PointCloud<PointT>::Ptr &cloud_in_out,
                                            double var_vertical_ang_d = 0.0,
                                            bool inverse_z = false) // inverse_z is for PANDAR XT Lidar
        {
            ...
            // typename pcl::PointCloud<PointT>::Ptr cloud_temp(new
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                // LOG(INFO)<<cloud_in_out->points[i].x << ","<< cloud_in_out->points[i].y
                // << ","<< cloud_in_out->points[i].z;
                ...
                // LOG(WARNING)<<cloud_in_out->points[i].x << ","<<
                // cloud_in_out->points[i].y << ","<< cloud_in_out->points[i].z;
            }
            // cloud_temp->points.swap(cloud_in_out->points);
        ...
        bool simple_motion_compensation(
            typename pcl::PointCloud<PointT>::Ptr &cloud_in_out,
            const Eigen::Matrix4d &estimated_tran2_1, // estimated_tran2_1 (from k to k+1 frame, the
                                                      // inverse of motion)
            double scan_begin_ang_anticlock_x_positive_deg = 180.0, double ambigous_angle_thre_deg = 1.0,
            bool only_rotation = false)
        // for velodyne HDL 64, the scan begin from the negative direction of x axis
        // (scan_begin_ang_anticlock_x_positive_deg = 180), spining anti-clockwise for
        // HESAI Pandar 64, the scan begin from the negative direction of y axis
        // (scan_begin_ang_anticlock_x_positive_deg = 270), spining anti-clockwise
        {
            typename pcl::PointCloud<PointT>::Ptr cloud_temp(new pcl::PointCloud<PointT>);
            double ang;
            double s; // interpolation ratio
            double scan_begin_ang_anticlock_x_positive_rad =
                scan_begin_ang_anticlock_x_positive_deg / 180.0 * M_PI;
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                ...
                if (ang >= 2 * M_PI)
                    ang -= 2 * M_PI;
                // --> transform to the scan begining direction, now ang is the
                // anti-clockwise angle from this begining direction

                // delete ambigous points (close to the begining or ending of a scan) -->
                // only add those non-ambigious points
                if (ang > ambigous_angle_thre_rad && ang < 2 * M_PI - ambigous_angle_thre_rad)
                {
                    ...
                    d_quat = Eigen::Quaterniond::Identity().slerp(
                        s, estimated_quat2_1); // Spherical linear interpolation (slerp) for
                                               // quaternion
                    d_translation = s * estimated_translation_2_1;
                    Eigen::Vector3d temp_point(cloud_in_out->points[i].x, cloud_in_out->points[i].y,
                                               cloud_in_out->points[i].z);
        ...
        // using uniform motion model to do motion compensation (with time stamp
        // (store timestamp as curvature))
        bool simple_motion_compensation_with_timestamp(
            typename pcl::PointCloud<PointT>::Ptr &cloud_in_out, const Eigen::Matrix4d &estimated_tran2_1,
            float scan_duration_min_ms =
                50) // estimated_tran2_1 (from k to k+1 frame, the inverse of motion)
        {
            typename pcl::PointCloud<PointT>::Ptr cloud_temp(new pcl::PointCloud<PointT>);
            double s; // interpolation ratio
            ...
            // get the first and last time stamp (stored as curvature) of this scan
            // (time stamp unit: ms)
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                s = (last_timestamp - cloud_in_out->points[i].curvature) /
                    scan_duration; // curvature as time stamp //TODO: fix
                // the first point (with earliest timestamp) would change the most
                // while the last point (with latest timestamp) would change the least
                //  if (i%10==0)
                //  LOG(INFO) << s;
        ...
        bool get_pts_timestamp_ratio_in_frame(typename pcl::PointCloud<PointT>::Ptr &cloud_in_out,
                                              ...
                                              double scan_begin_ang_anticlock_x_positive_deg = 180.0,
                                              float scan_duration_ms = 100)
        {
            double s; // interpolation ratio
            double ang;
            double scan_begin_ang_anticlock_x_positive_rad =
            ...
            if (timestamp_availiable)
            {
                for (int i = 0; i < cloud_in_out->points.size(); i++)
                {
                    last_timestamp = max_(last_timestamp,
                                          cloud_in_out->points[i].curvature); // ms as the unit
                    first_timestamp = min_(first_timestamp, cloud_in_out->points[i].curvature);
                }
                ...
                {
                    s = (last_timestamp - cloud_in_out->points[i].curvature) /
                        scan_duration_ms; // curvature as time stamp
                    cloud_in_out->points[i].curvature =
                        min_(1.0, max_(0.0, s)); // curvature as the time ratio in each frame
                }
                return true;
            }
            else // using azimuth to calculate timestamp ratio
            {
                for (int i = 0; i < cloud_in_out->points.size(); i++)
                {
                    ...
                    if (ang < 0)
                        ang += 2 * M_PI; // --> transform to the anti-clockwise angle from +x
                                         // axis [for HESAI, clockwise]
        ...
        // already get the timestamp ratio (stored as curvature) using
        // get_pts_timestamp_ratio_in_frame function
        void apply_motion_compensation(typename pcl::PointCloud<PointT>::Ptr pc_in_out,
                                       Eigen::Matrix4d &Tran, float s_ambigous_thre = 0.000)
        {
...
#pragma omp parallel for // Multi-thread
            ...
            {
                if (pc_in_out->points[i].curvature < s_ambigous_thre ||
                    pc_in_out->points[i].curvature > 1.0 - s_ambigous_thre) // curvature as the timestamp
                    continue;
                d_quat = Eigen::Quaterniond::Identity().slerp(
                    pc_in_out->points[i].curvature,
                    estimated_quat2_1); // Spherical linear interpolation (slerp) for
                                        // quaternion
                d_translation = pc_in_out->points[i].curvature * estimated_translation_2_1;
                Eigen::Vector3d temp_point(pc_in_out->points[i].x, pc_in_out->points[i].y,
                                           pc_in_out->points[i].z);
                Eigen::Vector3d undistort_point = d_quat * temp_point + d_translation;
        ...
        void apply_motion_compensation(const typename pcl::PointCloud<PointT>::Ptr pc_in,
...
#pragma omp parallel for // Multi-thread
            ...
            {
                if (pc_in->points[i].curvature < s_ambigous_thre ||
                    pc_in->points[i].curvature > 1.0 - s_ambigous_thre) // curvature as the timestamp
                    continue;
                d_quat = Eigen::Quaterniond::Identity().slerp(
                    pc_in->points[i].curvature,
                    estimated_quat2_1); // Spherical linear interpolation (slerp) for
                                        // quaternion
        ...
        // already get the timestamp ratio (stored as curvature) using
        // get_pts_timestamp_ratio_in_frame function
        void batch_apply_motion_compensation(typename pcl::PointCloud<PointT>::Ptr pc_ground,
                                             typename pcl::PointCloud<PointT>::Ptr pc_pillar,
                                             typename pcl::PointCloud<PointT>::Ptr pc_beam,
                                             typename pcl::PointCloud<PointT>::Ptr pc_facade,
                                             typename pcl::PointCloud<PointT>::Ptr pc_roof,
                                             typename pcl::PointCloud<PointT>::Ptr pc_vertex,
                                             Eigen::Matrix4d &Tran,
                                             bool undistort_keypoints_or_not = false)
        {
        ...
        // already get the timestamp ratio (stored as curvature) using
        // get_pts_timestamp_ratio_in_frame function
        void batch_apply_motion_compensation(const typename pcl::PointCloud<PointT>::Ptr pc_ground,
                                             const typename pcl::PointCloud<PointT>::Ptr pc_pillar,
                                             const typename pcl::PointCloud<PointT>::Ptr pc_beam,
                                             const typename pcl::PointCloud<PointT>::Ptr pc_facade,
                                             const typename pcl::PointCloud<PointT>::Ptr pc_roof,
                                             const typename pcl::PointCloud<PointT>::Ptr pc_vertex,
                                             typename pcl::PointCloud<PointT>::Ptr pc_ground_undistort,
                                             typename pcl::PointCloud<PointT>::Ptr pc_pillar_undistort,
                                             typename pcl::PointCloud<PointT>::Ptr pc_beam_undistort,
                                             typename pcl::PointCloud<PointT>::Ptr pc_facade_undistort,
                                             typename pcl::PointCloud<PointT>::Ptr pc_roof_undistort,
                                             typename pcl::PointCloud<PointT>::Ptr pc_vertex_undistort,
                                             Eigen::Matrix4d &Tran,
                                             bool undistort_keypoints_or_not = false)
        {
                                           ...
                                           int keep_number_per_sector, int sector_num)
        {
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                ...
                sectors[sector_id]->points.push_back(
                    cloud_in_out->points[i]); // push_back may cause collision for
            ...
            for (int j = 0; j < sector_num; j++)
            {
                ...
                cloud_temp->points.insert(cloud_temp->points.end(), sectors[j]->points.begin(),
                                          sectors[j]->points.end());
        ...
        // when keep_number == 0, the output point cloud would be empty (in other
        // words, the input point cloud would be cleared)
...

        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    ran_sample.filter(*cloud_temp);
                    cloud_temp->points.insert(cloud_temp->points.end(), cloud_high->points.begin(),
                                              cloud_high->points.end());
                    cloud_temp->points.swap(cloud_in_out->points);
        ...
        bool random_downsample(typename pcl::PointCloud<PointT>::Ptr &cloud_in_out,
        ...
        bool farthest_point_sampling(typename pcl::PointCloud<PointT>::Ptr &cloud_in_out,
                                     int downsample_ratio)
        {
            return 1;
            // TODO
        ...
        bool intensity_filter(
            typename pcl::PointCloud<PointT>::Ptr &cloud_in_out, float min_i_thre = 0,
            float max_i_thre = FLT_MAX,
            float intensity_scale = 255.0) // min_i_thre, max_i_thre are in the range [0,1]
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                if (cloud_in_out->points[i].intensity > min_i_thre * intensity_scale &&
                    cloud_in_out->points[i].intensity < min_i_thre * intensity_scale)
                    cloud_temp->points.push_back(cloud_in_out->points[i]);
            }
            ...
            // 		  << cloud_in_out->points.size() << " ] done in [" << time_used.count()
            // << "] s";
                                    ...
                                    float min_ang_thre = 0, float max_ang_thre = 90)
        {
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                dot_product_result = std::abs(cloud_in_out->points[i].x * cloud_in_out->points[i].normal_x +
                                              cloud_in_out->points[i].y * cloud_in_out->points[i].normal_y +
                                              cloud_in_out->points[i].z * cloud_in_out->points[i].normal_z);
                incidence_angle = std::acos(
                    dot_product_result / (std::sqrt(cloud_in_out->points[i].x * cloud_in_out->points[i].x +
                                                    cloud_in_out->points[i].y * cloud_in_out->points[i].y +
                                                    cloud_in_out->points[i].z * cloud_in_out->points[i].z)));
        ...
        // Filter the point cloud according to the horizontal distance to the scanner
        // (define dist_min and dist_max)
        bool dist_filter(typename pcl::PointCloud<PointT>::Ptr &cloud_in_out, double xy_dist_min,
                         double xy_dist_max)
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                dis_square = cloud_in_out->points[i].x * cloud_in_out->points[i].x +
                             cloud_in_out->points[i].y * cloud_in_out->points[i].y;
        ...
        // Filter the point cloud according to the horizontal and vertical distance to
        // the lidar center
        bool dist_filter(typename pcl::PointCloud<PointT>::Ptr &cloud_in_out, double xy_dis_thre,
                         bool keep_inside = true, double z_min = -DBL_MAX, double z_max = DBL_MAX)
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            ...
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                ...
                if (keep_inside)
                {
                    if (dis_square < xy_dis_thre * xy_dis_thre && cloud_in_out->points[i].z < z_max &&
                        cloud_in_out->points[i].z > z_min)
                    {
                        cloud_temp->points.push_back(cloud_in_out->points[i]);
                    }
                }
        ...
        // Filter the point cloud according to the horizontal and vertical distance to
        // the lidar center
        bool dist_filter(typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                         typename pcl::PointCloud<PointT>::Ptr &cloud_out, double xy_dis_thre,
                         bool keep_inside = true, double z_min = -DBL_MAX, double z_max = DBL_MAX)
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            ...
            for (int i = 0; i < cloud_in->points.size(); i++)
            {
                ...
                if (keep_inside)
                {
                    if (dis_square < xy_dis_thre * xy_dis_thre && cloud_in->points[i].z < z_max &&
            ...
            // LOG(INFO) << "Distance filter [ " << original_pts_num << " --> "
            // 		  << cloud_out->points.size() << " ] done in [" << time_used.count() <<
            // "] s";
        ...
        // we assume that most of the ghost points are underground outliers within a
        // radius from the laser scanner
        bool scanner_filter(const typename pcl::PointCloud<PointT>::Ptr &cloud_in_out, float self_radius,
                            float ghost_radius, float z_min_thre_ghost, float z_min_thre_global)
        {
            typename pcl::PointCloud<PointT>::Ptr cloud_temp(new pcl::PointCloud<PointT>);
            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                float dis_square = cloud_in_out->points[i].x * cloud_in_out->points[i].x +
                                   cloud_in_out->points[i].y * cloud_in_out->points[i].y;
                if (dis_square > self_radius * self_radius && cloud_in_out->points[i].z > z_min_thre_global)
                {
                    if (dis_square > ghost_radius * ghost_radius ||
                        cloud_in_out->points[i].z > z_min_thre_ghost)
                        cloud_temp->points.push_back(cloud_in_out->points[i]);
                }
            }
            cloud_temp->points.swap(cloud_in_out->points);
                        ...
                        typename pcl::PointCloud<PointT>::Ptr &cloud_out, bounds_t &bbx)
        {
            ...
            // LOG(INFO) << "# Points [" << cloud_in->points.size() << " -> bbx Filter
            // -> " << cloud_out->points.size() << " ]";

            return 1;
        }

        bool bbx_filter(const typename pcl::PointCloud<PointT>::Ptr &cloud_in_out, bounds_t &bbx,
                        bool delete_box = false)
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            typename pcl::PointCloud<PointT>::Ptr cloud_temp(new pcl::PointCloud<PointT>);
            int original_pts_num = cloud_in_out->points.size();
        ...
        bool active_object_filter_by_bbx(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                                         typename pcl::PointCloud<PointT>::Ptr &cloud_out,
                                         std::vector<bounds_t> &active_bbxs)
        {
            ...
            for (int i = 0; i < cloud_in->points.size(); i++)
            {
                ...
                {
                    // In the bounding box
                    if (cloud_in->points[i].x > active_bbxs[j].min_x &&
                        cloud_in->points[i].x < active_bbxs[j].max_x &&
                        cloud_in->points[i].y > active_bbxs[j].min_y &&
                        cloud_in->points[i].y < active_bbxs[j].max_y &&
                        cloud_in->points[i].z > active_bbxs[j].min_z &&
                        cloud_in->points[i].z < active_bbxs[j].max_z)
                    {
                        is_static[i] = 0;
                        break;
                    }
        ...
        // Brief: Detect some stable key points (vertex points) from the point cloud
        // according to the local curvature and stability
        bool detect_key_pts(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                            ...
                            double stable_ratio_max, float curvature_non_max_radius,
                            float min_curvature)
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            ...
            prune_unstable_pts(features, index_with_feature, stable_ratio_max, min_curvature,
                               candidateIndices);
            ...
            // LOG(INFO) << "Keypoint detection done (" <<
            // cloud_keypoints->points.size() << " keypoints)";
            return true;
        }

        // Brief: Get rid of some unstable points from the keypoints list
        bool prune_unstable_pts(const std::vector<pca_feature_t> &features,
                                const std::vector<int> &index_with_feature, float ratioMax,
                                float min_curvature, pcl::PointIndicesPtr &stable_indices,
...

        {
            for (int i = 0; i < features.size(); ++i)
            {
                ...
                if (ratio1 < ratioMax && ratio2 < ratioMax &&
                    features[i].pt_num > min_point_num_neighborhood &&
        ...
        bool encode_stable_points(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                                  ...
                                  const std::vector<int> &index_with_feature, float min_curvature = 0.0,
        ...
        // descriptor (ncc: neighborhood category context) at the same time
        {
            for (int i = 0; i < features.size(); ++i)
            {
                    ...
                    features[i].curvature > min_curvature)
                {
                    ...
                    int neighbor_total_count = 0, pillar_count = 0, beam_count = 0, facade_count = 0,
                        roof_count = 0;
                    int pillar_close_count = 0, pillar_far_count = 0, beam_close_count = 0, beam_far_count = 0,
                        facade_close_count = 0, facade_far_count = 0, roof_close_count = 0, roof_far_count = 0;
                    ...
                    int descriptor = pillar_count * 1000000 + beam_count * 10000 + facade_count * 100 +
                                     roof_count; // the neighborhood discriptor (8 numbers)
                    int descriptor_1 = pillar_close_count * 1000000 + beam_close_count * 10000 +
                                       facade_close_count * 100 + roof_close_count;
                    ...
                    pt.intensity =
                        accu_intensity / neighbor_total_count; // mean intensity of the nrighborhood
                                                               // pt.normal[3] store the point curvature
                                                               // pt.data[3] store the height of the point
                                                               // above the ground

                    //!!! TODO: fix, use customed point type, you need a lot of porperties
                    //! for saving linearity, planarity, curvature, semantic label and
                    //! timestamp
                    //!!! However, within the template class, there might be a lot of
                    //! problems (waiting for the code reproducing)
        ...
        bool non_max_suppress(
            typename pcl::PointCloud<PointT>::Ptr &cloud_in_out, float non_max_radius,
            bool kd_tree_already_built = false,
            const typename pcl::search::KdTree<PointT>::Ptr &built_tree = NULL) // according to curvature
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            typename pcl::PointCloud<PointT>::Ptr cloud_temp(new pcl::PointCloud<PointT>());
            ...
            std::sort(cloud_in_out->points.begin(), cloud_in_out->points.end(),
            ...
            // typename pcl::KdTreeFLANN<PointT>::Ptr tree(new
            ...
            // LOG(INFO) << "NMS done from [" << pt_count_before << "] to [" << pt_count_after_nms
            //           << "] points in [" << 1000.0 * time_used.count() << "] ms.";
        ...
        bool non_max_suppress(
            ...
            bool distance_adaptive_on = false, float unit_dist = 35.0, bool kd_tree_already_built = false,
            const typename pcl::search::KdTree<PointT>::Ptr &built_tree = NULL) // according to curvature
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            typename pcl::PointCloud<PointT>::Ptr cloud_temp(new pcl::PointCloud<PointT>());
            ...
            // pt_count_after_nms << "] points in [" << 1000.0 * time_used.count() << "]
            // ms.";
        ...
        bool non_max_suppress(std::vector<pca_feature_t> &features, pcl::PointIndicesPtr &indices,
                              float nms_radius)
        {
            std::sort(features.begin(), features.end(), [](const pca_feature_t &a, const pca_feature_t &b)
        ...
        bool detect_curbs(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                          typename pcl::PointCloud<PointT>::Ptr &cloud_curb)
        {
            ...
            for (int i = 0; i < ring_number; i++)
                rings[i] = std::make_shared<typename pcl::PointCloud<PointT>>();

            for (int i = 0; i < cloud_in->points.size(); i++)
                rings[(unsigned int)(cloud_in->points[i].curvature)]->points.push_back(
                    cloud_in->points[i]); // curvature store the ring number here

            // cloud_curb = rings[40];

            for (int i = 0; i < ring_number; i++)
            {
                // LOG(INFO) << "Ring [" << i << "]: [" << rings[i]->points.size() << "]
                // ground points.";
                if (rings[i]->points.size() > ring_pts_count_thre && i > min_ring_num && i < max_ring_num)
                    detect_curbs_on_scanline(rings[i],
                                             cloud_curb); // append into cloud_curb
            }

            return true;
        }

        static bool compare_hor_ang(const PointT &pt_a,
                                    const PointT &pt_b) // from 0 to 360
        {
            return pt_a.curvature < pt_b.curvature; // now curvature is used to store
                                                    // the horizontal angle
        }

        // detect curb points on each scanline (Deprecated)
        bool detect_curbs_on_scanline(typename pcl::PointCloud<PointT>::Ptr &ring,
                                      typename pcl::PointCloud<PointT>::Ptr &cloud_curb,
                                      float delta_z_thre = 0.03, float delta_z_thre_max = 0.08,
                                      float scan_begin_ang_anticlock_x_positive_deg = 180.0)
        {
            ...
            float hor_ang_deg;
            for (int i = 0; i < ring->points.size();
                 i++) // now curvature is used to store the horizontal angle
            {
            ...
            {
                float hor_ang_window =
                    ring->points[i + window_size - 1].curvature - ring->points[i].curvature;
                bool is_spining_begin_end =
                    (ring->points[i].curvature >
                         (scan_begin_ang_anticlock_x_positive_deg - 2 * hor_ang_deg_diff_thre) &&
                     ring->points[i + window_size - 1].curvature <
                         (scan_begin_ang_anticlock_x_positive_deg + 2 * hor_ang_deg_diff_thre));
                ...
                {
                    // float delta_z = std::fabs(ring->points[i + j].z - ring->points[i + j
                    // + 1].z); if (delta_z > delta_z_max)
                    // {
                    // 	delta_z_max = delta_z;
                    // 	candidate_curb_idx = i + j; //record candidate curb point in the
                    // window
                    // }
                ...
                if (window_delta_z >= z_thresh_min && window_delta_z < z_thresh_max) // candidate curb window
                {
                    ...
                    for (int j = 0; j < window_size - 1; j++)
                    {
                        ...
                        float adjacent_hor_dist2 = (ring->points[i + j].y - ring->points[i + j + 1].y) *
                                                       (ring->points[i + j].y - ring->points[i + j + 1].y) +
                                                   (ring->points[i + j].x - ring->points[i + j + 1].x) *
                                                       (ring->points[i + j].x - ring->points[i + j + 1].x);
            ...
            // LOG(INFO) << "Ring [" << ring_id << "]: [" << curb_count << "] curb
            // points.";

            cloud_curb->points.insert(cloud_curb->points.end(), ring_curb_pts->points.begin(),
                                      ring_curb_pts->points.end());

            return true;
        }

        // simple implement of bi-threshold ground segmentation without further
        ...
        bool ground_seg(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                        ...
                        typename pcl::PointCloud<PointT>::Ptr &cloud_unground, int min_grid_pt_num,
                        float grid_resolution, float max_height_difference, float neighbor_height_diff,
                        float max_ground_height, float min_ground_height)
        {
            ...
            this->get_cloud_bbx_cpt(cloud_in, bounds,
                ...
                if (temp_id >= 0 && temp_id < num_grid)
                {
                    ...
                    else
                    {
                        ...
                        if (cloud_in->points[j].z < grid[temp_id].min_z &&
                            cloud_in->points[j].z > min_ground_height)
                        {
                    ...
                    if (grid[i].min_z - grid[i].neighbor_min_z < neighbor_height_diff)
                    {
                        for (int j = 0; j < grid[i].point_id.size(); j++)
                        {
                            if (cloud_in->points[grid[i].point_id[j]].z - grid[i].min_z < max_height_difference &&
                                ...
                                cloud_ground->points.push_back(
                                    cloud_in->points[grid[i].point_id[j]]); // Add to ground points
                            else
                                cloud_unground->points.push_back(
                                    cloud_in->points[grid[i].point_id[j]]); // Add to nonground points
                        }
                    }
                    else
                    {
        ...
        // 3.For each grid, if its 8 neighbor grids' Minimum Z value is less than
        // current grid's Minimum Z minus threshold1, then all the points in current
        // grid would be seen as unground points
        // 4.Or, points whose Z value is larger
        // than grid's Minimum Z plus threshold2 would be regarded as unground points.
        // The rest points in the grid would be ground points. (Estimate Ground
        // Points' normal at the same time) for multiple scan line mobile scanning
        // data, we can detect the curb points at the same time according to ring
        // information ground_random_down_rate: 1 ground point in
        // [ground_random_down_rate] ground points of each grid would be kept (for
        // example, 20) ground_random_down_down_rate: 1 ground point in
        // [ground_random_down_down_rate] ground points of the already downsampled
        // ground points would be kept (for example, 2) Reference paper: Two-step
        // adaptive extraction method for ground points and breaklines from lidar
        // point clouds, ISPRS-J, Yang.B, Huang.R, et al.
        // -------------------------------------------------------------------------------------------------------------------//
        bool fast_ground_filter(
            ...
            typename pcl::PointCloud<PointT>::Ptr &cloud_curb, int min_grid_pt_num, float grid_resolution,
            float max_height_difference, float neighbor_height_diff, float max_ground_height,
            int ground_random_down_rate, int ground_random_down_down_rate, int nonground_random_down_rate,
            int reliable_neighbor_grid_num_thre, int estimate_ground_normal_method,
            float normal_estimation_radius, // estimate_ground_normal_method, 0:
                                            // directly use (0,0,1), 1: estimate normal
                                            // in fix radius neighborhood , 2: estimate
                                            // normal in k nearest neighborhood, 3: use
                                            // ransac to estimate plane coeffs in a grid
            ...
            bool apply_grid_wise_outlier_filter = false,
            float outlier_std_scale =
                3.0) // current intensity_thre is for kitti dataset (TODO: disable it)
        {
            std::chrono::steady_clock::time_point tic = std::chrono::steady_clock::now();
            ...
            non_ground_height_thre = max_ground_height;
            // LI_INFO("non_ground_height_thre: {} = {} + {}", non_ground_height_thre, appro_mean_height,
            //         max_ground_height);
            ...
            // this is a keyparameter.
            // underground_noise_thre = appro_mean_height - max_ground_height;
            underground_noise_thre = -2.0;

            bounds_t bounds;
            centerpoint_t center_pt;
            // Inherited from its parent class, use this->
            ...
            for (int j = 0; j < cloud_in->points.size(); j++)
            {
                ...
                if (temp_id >= 0 && temp_id < num_grid)
                {
                    ...
                    {
                        grid[temp_id].dist2station = std::sqrt(cloud_in->points[j].x * cloud_in->points[j].x +
                    ...
                    if (cloud_in->points[j].z > non_ground_height_thre)
                    {
                        ...
                        distance_weight = 1.0 * standard_distance / (grid[temp_id].dist2station + 0.0001);
                        int nonground_random_down_rate_temp = nonground_random_down_rate;
                        if (distance_weight_downsampling_method == 1) // linear weight
                            ...
                            nonground_random_down_rate_temp =
                                (int)(distance_weight * distance_weight * nonground_random_down_rate + 1);
            ...
            {
                // Each grid: Check outlier //calculate mean and standard deviation of z
                ...
                for (int i = 0; i < num_grid; i++)
                {
                    if (grid[i].pts_count >= min_grid_pt_num)
                    {
                        ...
                        for (int j = 0; j < grid[i].point_id.size(); j++)
                            sum_z2 += (cloud_in->points[grid[i].point_id[j]].z - mean_z) *
                                      (cloud_in->points[grid[i].point_id[j]].z - mean_z);
...
#pragma omp parallel for
                    ...
                    grid[i].reliable_neighbor_grid_num >= reliable_neighbor_grid_num_thre)
                {
                    ...
                    else if (distance_weight_downsampling_method == 2) // quadratic weight
                    {
                        ground_random_down_rate_temp =
                            (int)(distance_weight * distance_weight * ground_random_down_rate + 1);
                        nonground_random_down_rate_temp =
                            (int)(distance_weight * distance_weight * nonground_random_down_rate + 1);
                    }
                                ...
                                else // inner grid unground points
                                {
                                    if (j % nonground_random_down_rate_temp == 0 ||
                                        cloud_in->points[grid[i].point_id[j]].intensity >
                                            intensity_thre) // extract more points on signs and
                                                            // vehicle license plate
                                    {
                                        cloud_in->points[grid[i].point_id[j]].data[3] =
                                            cloud_in->points[grid[i].point_id[j]].z -
                                            grid[i].min_z; // data[3] stores the point height above
                                                           // ground
                        ...
                        for (int j = 0; j < grid[i].point_id.size(); j++)
                        {
                            if (cloud_in->points[grid[i].point_id[j]].z > grid[i].min_z_outlier_thre &&
                                (j % nonground_random_down_rate_temp == 0 ||
                                 cloud_in->points[grid[i].point_id[j]].intensity > intensity_thre))
                            {
                                cloud_in->points[grid[i].point_id[j]].data[3] =
                                    cloud_in->points[grid[i].point_id[j]].z -
                                    grid[i].neighbor_min_z; // data[3] stores the point height
                                                            // above ground
                                grid_unground_pcs[i]->points.push_back(cloud_in->points[grid[i].point_id[j]]);
                                // cloud_unground->points.push_back(cloud_in->points[grid[i].point_id[j]]);
                                // //Add to nonground points
                    ...
                    if (estimate_ground_normal_method == 3 && grid_ground->points.size() >= min_grid_pt_num)
                    {
                        ...
                        // r is the inlier ratio (> 0.75 in our case), N is 3 in our case (3
                        // points can fit a plane), to get a confidence > 0.99, we need about
                        // 20 iteration (M=20)
                        estimate_ground_normal_by_ransac(grid_ground, 0.3 * max_height_difference, 20, normal_x,
                                                         normal_y, normal_z);
                        ...
                        std::chrono::duration<double> ground_ransac_time_per_grid =
                            std::chrono::duration_cast<std::chrono::duration<double>>(toc_ransac - tic_ransac);
            ...
            for (int i = 0; i < num_grid; i++)
            {
                cloud_ground->points.insert(cloud_ground->points.end(), grid_ground_pcs[i]->points.begin(),
            ...
            for (int i = 0; i < cloud_ground->points.size(); i++)
            {
                ...
                {
                    // LOG(INFO)<<cloud_ground->points[i].normal_x << "," <<
                    // cloud_ground->points[i].normal_y << "," <<
                    // cloud_ground->points[i].normal_z;
            ...
            // LOG(INFO) << "Ground: [" << cloud_ground->points.size() << " | "
            //           << cloud_ground_down->points.size() << "] Unground: ["
            //           << cloud_unground->points.size() << "].";

/*
    if (estimate_ground_normal_method == 3) {
      // LOG(INFO) << "Ground segmentation done in [" << ground_seg_time.count()
      // * 1000.0 - consuming_time_ransac << "] ms."; LOG(INFO) << "Ground
      // Normal Estimation done in [" << consuming_time_ransac << "] ms.";
      LOG(INFO) << "Ground segmentation and normal estimation in ["
                << ground_seg_time.count() * 1000.0 << "] ms."
                << ",in which preparation costs [" << ground_seg_prepare_time.count() * 1000.0
                << "] ms.";
      // output detailed consuming time
      // LOG(INFO) << prepare_1.count() * 1000.0 << "," << prepare_2.count() *
      // 1000.0 << "," << prepare_3.count() * 1000.0 << "," << prepare_4.count()
      // * 1000.0 << "," << prepare_5.count() * 1000.0;
    } else {
      LOG(INFO) << "Ground segmentation done in [" << ground_seg_time.count() * 1000.0 << "] ms.";
      LOG(INFO) << "Ground Normal Estimation done in [" << ground_normal_time.count() * 1000.0
                << "] ms."
    ...
    */
        ...
        bool estimate_ground_normal_by_ransac(typename pcl::PointCloud<PointT>::Ptr &grid_ground,
                                              float dist_thre, int max_iter, float &nx, float &ny,
                                              float &nz)
        {
        ...
        bool classify_nground_pts(typename pcl::PointCloud<PointT>::Ptr &cloud_in,
                                  ...
                                  float neighbor_searching_radius,
                                  int neighbor_k,
                                  int neigh_k_min,
                                  ...
                                  float edge_thre,
                                  float planar_thre,
                                  float edge_thre_down,
                                  float planar_thre_down,
                                  int extract_vertex_points_method,
                                  float curvature_thre,
                                  float vertex_curvature_non_max_radius,
                                  float linear_vertical_sin_high_thre,
                                  float linear_vertical_sin_low_thre,
                                  float planar_vertical_sin_high_thre,
                                  float planar_vertical_sin_low_thre,
                                  bool fixed_num_downsampling = false,
                                  int pillar_down_fixed_num = 200,
                                  int facade_down_fixed_num = 800,
                                  int beam_down_fixed_num = 200,
                                  int roof_down_fixed_num = 100,
                                  int unground_down_fixed_num = 20000,
                                  float beam_height_max = FLT_MAX,
                                  float roof_height_min = -FLT_MAX,
                                  float feature_pts_ratio_guess = 0.3,
                                  bool sharpen_with_nms = true,
...

        {
            ...
            pca_estimator.get_pc_pca_feature(cloud_in, cloud_features, tree, neighbor_searching_radius,
                                             neighbor_k, 1, pca_down_rate, use_distance_adaptive_pca,
                                             unit_distance);
            ...
            std::vector<int> index_with_feature(cloud_in->points.size(),
                                                0); // 0 - not special points, 1 - pillar, 2 -
                                                    // beam, 3 - facade, 4 - roof
            ...
            if (extract_vertex_points_method == 2)
            {
                ...
                for (int i = 0; i < cloud_in->points.size(); i++)
                {
                    ...
                    if (index_with_feature[i] == 0 && cloud_features[i].pt_num > neigh_k_min &&
                        cloud_features[i].curvature >
                            curvature_thre) // curvature_thre means curvature_thre here
                    {
                        ...
                        if (1.0 * geo_feature_point_count / cloud_features[i].pt_num >
                            vertex_feature_ratio_thre) // most of the neighbors are feature
                                                       // points
                        {
                            // cloud_vertex->points.push_back(cloud_in->points[i]);
                            ...
                            cloud_in->points[i].normal[3] =
                                5.0 * cloud_features[i].curvature; // save in the un-used normal[3] (PointNormal4D)
                            if (std::abs(cloud_features[i].vectors.principalDirection.z()) >
                                linear_vertical_sin_high_thre)
                            {
                                cloud_pillar->points.push_back(cloud_in->points[i]);
            ...
            // if extract_vertex_points_method == 0 ---> do not extract vertex points
            // (0)
            std::chrono::steady_clock::time_point toc_1 = std::chrono::steady_clock::now();
            ...
            encode_stable_points(cloud_in, cloud_vertex, cloud_features, index_with_feature,
                                 ...
                                 neigh_k_min); // encode the keypoints, we will get a simple descriptor
                                               // of the putable keypoints
...
#pragma omp section
            ...
            if (fixed_num_downsampling)
            {
                ...
                int sector_num = 4;
                xy_normal_balanced_downsample(cloud_facade_down, (int)(facade_down_fixed_num / sector_num),
                                              sector_num);

                xy_normal_balanced_downsample(cloud_beam_down, (int)(beam_down_fixed_num / sector_num),
                                              sector_num); // here the normal is the primary vector
            ...
            std::chrono::duration<double> time_used_extract_geo_pts =
                std::chrono::duration_cast<std::chrono::duration<double>>(toc_1 - toc_pca);
            ...
            /*
                LOG(INFO) << "Unground geometric feature points extracted done in ["
                          << time_used_all.count() * 1000.0 << "] ms.";
                LOG(INFO) << "Details: pca in [" << time_used_pca.count() * 1000.0
                          << "] ms, geometric feature points extracted in ["
                          << time_used_extract_geo_pts.count() * 1000.0 << "] ms, encoding keypoints in ["
                          << time_used_encoding_key_pts.count() * 1000.0 << "] ms, nms sharpen in ["
                          << time_used_nms_sharping.count() * 1000.0 << "] ms, downsampling in ["
                          << time_used_fixed_num_downsampling.count() * 1000.0 << "] ms.";
                LOG(INFO) << "Pillar: [" << cloud_pillar->points.size() << " | "
                          << cloud_pillar_down->points.size() << "] Beam: [" << cloud_beam->points.size()
                          << " | " << cloud_beam_down->points.size() << "] Facade: ["
                          << cloud_facade->points.size() << " | " << cloud_facade_down->points.size()
                          << "] Roof: [" << cloud_roof->points.size() << " | " <<
               cloud_roof_down->points.size()
                          << "] Vertex: [" << cloud_vertex->points.size() << "].";
            */
            return 1;
        }
        ...
        bool extract_semantic_pts(cloudblock_Ptr in_block,
                                  float vf_downsample_resolution,
                                  float gf_grid_resolution,
                                  float gf_max_grid_height_diff,
                                  float gf_neighbor_height_diff,
                                  float gf_max_ground_height,
                                  int &gf_down_rate_ground,
                                  int &gf_downsample_rate_nonground,
                                  float pca_neighbor_radius,
                                  int pca_neighbor_k,
                                  float edge_thre,
                                  float planar_thre,
                                  float curvature_thre,
                                  ...
                                  float planar_thre_down,
                                  bool use_distance_adaptive_pca = false,
                                  int distance_inverse_sampling_method = 0, // distance_inverse_downsample, 0: disabled, 1:
                                                                            // linear weight, 2: quadratic weight
                                  float standard_distance = 15.0,           // the distance where the weight is 1, only useful when
                                                                            // distance_inverse_downsample is on
                                  int estimate_ground_normal_method = 3,    // estimate_ground_normal_method, 0: directly use (0,0,1), 1:
                                                                            // estimate normal in fix radius neighborhood , 2: estimate normal
                                                                            // in k nearest neighborhood, 3: use ransac to estimate plane
                                                                            // coeffs in a grid
                                  ...
                                  int pca_down_rate = 1,
                                  float intensity_thre = FLT_MAX, // default intensity_thre means that highly-reflective
                                                                  // objects would not be prefered
                                  ...
                                  float roi_max_y = 0.0)
        {
            ...
            // with semantic mask
            // remove dynamic objects and outliers (underground ghost points) using the
            // semantic mask predicted by neural network
            ...
            else if (apply_scanner_filter)
            {
                ...
                // filter the point cloud of the back of the vehicle itself and the
                // underground
                scanner_filter(in_block->pc_raw, self_ring_radius, ghost_radius, z_min, z_min_min);
                // dist_filter(in_block->pc_raw, self_ring_radius, false, z_min); //
                // filter the point cloud of the back of the vehicle itself
            }
            ...
            random_downsample(in_block->pc_down, in_block->pc_sketch,
                              in_block->pc_down->points.size() / 1024 + 1);

            // Used for UAV borne Lidar SLAM
            // rotate to world coordiante system (because we want to use some geometric
            // constraints that would only apply on world coordinate system) typename
            // pcl::PointCloud<PointT>::Ptr rotated_pc_down(new
            // pcl::PointCloud<PointT>); pcl::transformPointCloud(*in_block->pc_down,
            // *rotated_pc_down, in_block->pose_lo); //now the pose_lo stored only the
            // rotation matrix of last frame
            // filtered point cloud --> ground & non-ground point cloud
            fast_ground_filter(
                in_block->pc_down, in_block->pc_ground, in_block->pc_ground_down, in_block->pc_unground,
                in_block->pc_vertex, gf_grid_pt_num_thre, gf_grid_resolution, gf_max_grid_height_diff,
                gf_neighbor_height_diff, gf_max_ground_height, gf_down_rate_ground,
                gf_down_down_rate_ground, gf_downsample_rate_nonground, gf_reliable_neighbor_grid_thre,
                estimate_ground_normal_method, normal_estimation_radius, distance_inverse_sampling_method,
                standard_distance, fixed_num_downsampling, ground_down_fixed_num, extract_curb_or_not,
            ...
            /*
                static int cnt = 0;
                if (!in_block->pc_ground->empty())
                  pcl::io::savePCDFileBinary("/home/lzz/pointcloud/ground/" + std::to_string(cnt) + ".pcd",
                                             *in_block->pc_ground);
                if (!in_block->pc_unground->empty())
                  pcl::io::savePCDFileBinary("/home/lzz/pointcloud/unground/" + std::to_string(cnt) +
               ".pcd", *in_block->pc_unground); cnt++;
            */
		...
		if (apply_roi_filtering) 
		{
            ...
            // non-ground points --> planar (facade, roof) & linear (pillar, beam) &
            // spherical (vertex) points
            classify_nground_pts(
                in_block->pc_unground, in_block->pc_pillar, in_block->pc_beam, in_block->pc_facade,
                in_block->pc_roof, in_block->pc_pillar_down, in_block->pc_beam_down,
                in_block->pc_facade_down, in_block->pc_roof_down, in_block->pc_vertex, pca_neighbor_radius,
                pca_neighbor_k, pca_neighbor_k_min, pca_down_rate, edge_thre, planar_thre, edge_thre_down,
                planar_thre_down, extract_vertex_points_method, curvature_thre, vertex_curvature_non_max_r,
                linear_vertical_sin_high_thre, linear_vertical_sin_low_thre, planar_vertical_sin_high_thre,
                planar_vertical_sin_low_thre, fixed_num_downsampling, pillar_down_fixed_num,
                ...
                use_distance_adaptive_pca);

            // with semantic mask
            // using semantic mask predicted by neural network to refine the detected
            ...
            in_block->down_feature_point_num =
                in_block->pc_ground_down->points.size() + in_block->pc_pillar_down->points.size() +
                in_block->pc_beam_down->points.size() + in_block->pc_facade_down->points.size() +
                in_block->pc_roof_down->points.size() + in_block->pc_vertex->points.size();

            // update the parameters according to the situation
            if (use_adpative_parameters)
                update_parameters_self_adaptive(
                    gf_down_rate_ground, gf_downsample_rate_nonground, pca_neighbor_radius, edge_thre,
                    planar_thre, edge_thre_down, planar_thre_down, in_block->pc_ground_down->points.size(),
                    in_block->pc_facade_down->points.size(), in_block->pc_pillar_down->points.size(),
                    in_block->pc_beam_down->points.size());
        ...
        void update_parameters_self_adaptive(
            int &gf_down_rate_ground, int &gf_downsample_rate_nonground, float &pca_neighbor_k,
            ...
            int facade_down_num_expected = 400, int pillar_down_num_expected = 200,
            int beam_down_num_expected = 300)
        {
            // int non_ground_feature_num = facade_down_num + beam_down_num +
            // pillar_down_num;
            ...
            // 	gf_down_rate_ground = max_(1, gf_down_rate_ground -
            // ground_down_num_min_expected / ground_down_num);

            // if (non_ground_feature_num > non_ground_num_max_expected)
            // {
            // 	gf_downsample_rate_nonground = min_(6, gf_downsample_rate_nonground +
            // non_ground_feature_num / non_ground_num_max_expected);
            // 	//pca_neighbor_k = min_(8, pca_neighbor_k - 1);
            // }

            if (non_ground_feature_num < non_ground_num_min_expected)
            {
                gf_downsample_rate_nonground = max_(
                    1, gf_downsample_rate_nonground - non_ground_num_min_expected / non_ground_feature_num);
                                            ...
                                            int pillar_down_down_rate, int beam_down_down_rate)
        {
            ...
            // ground: 40 (road), 44 (parking), 48 (sidewalk), 49 (other-ground), 60
            // (lane-marking), 72 (terrian) facade: 50 (building), 51 (fence), 13 (bus)
            ...
            for (int i = 0; i < in_block->pc_down->points.size(); i++)
            {
                ...
                // if (i % 100 == 0)
                // 	std::cout << "example: point 10 (" <<
                // in_block->pc_down->points[i].normal_x << "," <<
                // in_block->pc_down->points[i].normal_y << "," <<
                // in_block->pc_down->points[i].normal_z << ").\n";
            }
            ...
            // random_downsample(in_block->pc_beam, in_block->pc_beam_down,
            // beam_down_down_rate);
            return 1;
        }
        ...
        // Only works on Semantic KITTI dataset (Deprecated)
        // http://semantic-kitti.org/dataset.html
        // actually, points that are farther than 50 m from the scanner would not be
        // labeled. This would cause a lot of problem
        bool filter_with_dynamic_object_mask_pre(
            const typename pcl::PointCloud<PointT>::Ptr &cloud_in_out)
        {
            typename pcl::PointCloud<PointT>::Ptr cloud_temp(new pcl::PointCloud<PointT>);

            for (int i = 0; i < cloud_in_out->points.size(); i++)
            {
                if (cloud_in_out->points[i].curvature < 250 &&
                    cloud_in_out->points[i].curvature != 1) // block moving objects and outliers by semantic
                                                            // semantic_kitti masks
                    cloud_temp->points.push_back(cloud_in_out->points[i]);
                // 	 252: "moving-car"  //   253: "moving-bicyclist"  //   254:
                // "moving-person"
                //   255: "moving-motorcyclist" //   256: "moving-on-rails" //   257:
                //   "moving-bus" 258: "moving-truck" //   259: "moving-other-vehicle"
        ...
        // Only works on Semantic KITTI dataset (Deprecated)
        // http://semantic-kitti.org/dataset.html
        void filter_with_semantic_mask(cloudblock_Ptr in_block,
                                       const std::string mask_feature_type = "000000")
        {
            ...
            if (mask_feature_type[0] == '1') // ground
            {
                ...
                for (int i = 0; i < in_block->pc_ground->points.size(); i++)
                {
                    ...
                    float dist2 = in_block->pc_ground->points[i].x * in_block->pc_ground->points[i].x +
                                  in_block->pc_ground->points[i].y * in_block->pc_ground->points[i].y;
                    if (label == 40 || label == 44 || label == 48 || label == 49 || label == 72 ||
                        dist2 > labeled_radius * labeled_radius)
                ...
                for (int i = 0; i < in_block->pc_ground_down->points.size(); i++)
                {
                    ...
                    float dist2 =
                        in_block->pc_ground_down->points[i].x * in_block->pc_ground_down->points[i].x +
                        in_block->pc_ground_down->points[i].y * in_block->pc_ground_down->points[i].y;
                    if (label == 40 || label == 44 || label == 48 || label == 49 || label == 72 ||
                        dist2 > labeled_radius * labeled_radius)
            ...
            if (mask_feature_type[2] == '1') // facade
            {
                ...
                for (int i = 0; i < in_block->pc_facade->points.size(); i++)
                {
                    ...
                    float dist2 = in_block->pc_facade->points[i].x * in_block->pc_facade->points[i].x +
                                  in_block->pc_facade->points[i].y * in_block->pc_facade->points[i].y;
                ...
                for (int i = 0; i < in_block->pc_facade_down->points.size(); i++)
                {
                    ...
                    float dist2 =
                        in_block->pc_facade_down->points[i].x * in_block->pc_facade_down->points[i].x +
                        in_block->pc_facade_down->points[i].y * in_block->pc_facade_down->points[i].y;
                    if (label == 50 || label == 13 || label == 51 || dist2 > labeled_radius * labeled_radius)
            ...
            if (mask_feature_type[1] == '1') // pillar
            {
                ...
                for (int i = 0; i < in_block->pc_pillar->points.size(); i++)
                {
                    ...
                    float dist2 = in_block->pc_pillar->points[i].x * in_block->pc_pillar->points[i].x +
                                  in_block->pc_pillar->points[i].y * in_block->pc_pillar->points[i].y;
                ...
                for (int i = 0; i < in_block->pc_pillar_down->points.size(); i++)
                {
                    ...
                    float dist2 =
                        in_block->pc_pillar_down->points[i].x * in_block->pc_pillar_down->points[i].x +
                        in_block->pc_pillar_down->points[i].y * in_block->pc_pillar_down->points[i].y;
                    if (label == 71 || label == 80 || label == 81 || dist2 > labeled_radius * labeled_radius)
            ...
            if (mask_feature_type[3] == '1') // beam
            {
                ...
                for (int i = 0; i < in_block->pc_beam->points.size(); i++)
                {
                    ...
                    float dist2 = in_block->pc_beam->points[i].x * in_block->pc_beam->points[i].x +
                                  in_block->pc_beam->points[i].y * in_block->pc_beam->points[i].y;
                ...
                for (int i = 0; i < in_block->pc_beam_down->points.size(); i++)
                {
                    ...
                    float dist2 = in_block->pc_beam_down->points[i].x * in_block->pc_beam_down->points[i].x +
                                  in_block->pc_beam_down->points[i].y * in_block->pc_beam_down->points[i].y;
                    if (label == 50 || label == 51 || label == 10)
            ...
            /*
                LOG(INFO) << "Feature point number after semantic mask filtering: "
                          << "Ground: [" << in_block->pc_ground->points.size() << " | "
                          << in_block->pc_ground_down->points.size() << "] Pillar: ["
                          << in_block->pc_pillar->points.size() << " | "
                          << in_block->pc_pillar_down->points.size() << "] Facade: ["
                          << in_block->pc_facade->points.size() << " | "
                          << in_block->pc_facade_down->points.size() << "] Beam: ["
                          << in_block->pc_beam->points.size() << " | " <<
               in_block->pc_beam_down->points.size()
                          << "]";
              */
        }
        ...
        // Use the intersection bounding box to filter the outlier points (--> to
        // speed up)
        ...
        // speed up)
        bool get_cloud_pair_intersection(bounds_t &intersection_bbx,
                                         typename pcl::PointCloud<PointT>::Ptr &pc_target,
                                         typename pcl::PointCloud<PointT>::Ptr &pc_source)
        {
        ...
        // Brief: Generate the crosssection of the point cloud for better
...
#if OPENCV_ON
        bool pointcloud_to_rangeimage(typename pcl::PointCloud<PointT>::Ptr &point_cloud,
                                      cv::Mat &range_image)
        {
            ...
            for (int i = 0; i < point_cloud->points.size(); i++)
            {
                ...
                // if (i % 100 == 0)
                //     std::cout << r << "," << c << "," << point_cloud->points[i].d <<
                //     std::endl;
        ...
        cv::Mat generate_2d_map(const typename pcl::PointCloud<PointT>::Ptr &cloud, double m2pix,
                                int map_width, int map_height, int min_points_in_pix,
                                int max_points_in_pix, double min_height, double max_height,
                                bool shift_or_not = false)
        {
