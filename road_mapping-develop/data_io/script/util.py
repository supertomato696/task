...
class log:
    ...
    def info(cls, *args):
        ...
        np.set_printoptions(precision=6)  # 精度为 6 位
        print(cls.time_str(), *args)

    @classmethod
    def debug(cls, *args):
        np.set_printoptions(suppress=True)  # 不使用科学计数法
        np.set_printoptions(precision=6)  # 精度为 6 位
        print("\033[92m" + cls.time_str(), *args, "\033[00m")

    @classmethod
    def warning(cls, *args):
        np.set_printoptions(suppress=True)  # 不使用科学计数法
        np.set_printoptions(precision=6)  # 精度为 6 位
        print("\033[93m" + cls.time_str(), *args, "\033[00m")

    @classmethod
    def error(cls, *args):
        np.set_printoptions(suppress=True)  # 不使用科学计数法
        np.set_printoptions(precision=6)  # 精度为 6 位
        print("\033[91m" + cls.time_str(), *args, "\033[00m")
