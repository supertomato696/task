...
double calc_vertical_dis(const Eigen::Vector3d &from, const Eigen::Vector3d &to, const Eigen::Vector3d &dir,
    ...
    // 如果dir的长度为0，则返回一个较大的距离（不进行计算）
    if (diff_sqr_norm <= 0.0) {
        return 1000;
    }

    // 计算(from - to)向量在dir方向上的投影比例
    ...
    // 如果需要考虑方向（need_dir），则判断目标点在左侧还是右侧
    if (need_dir) {
...
// 计算向量f1f2与t1t2之间的交点
bool get_cross_point_for_segment(const Eigen::Vector3d &f1, const Eigen::Vector3d &f2,
        const Eigen::Vector3d &t1, const Eigen::Vector3d &t2, Eigen::Vector3d &ret, int mode, bool ignore_z) {
    // z 值根据from点延伸
    ...
    if (mode == 1) {
        Eigen::Vector3d to_dir = get_dir(t2, t1, ignore_z);
        Eigen::Vector3d t1_t = t1 - 1000 * to_dir;
        Eigen::Vector3d t2_t = t2 + 1000 * to_dir;
        p_to1 = {t1_t.x(), t1_t.y()};
        p_to2 = {t2_t.x(), t2_t.y()};
    }

    if (mode == 2) {
        Eigen::Vector3d from_dir = get_dir(f2, f1, ignore_z);
        Eigen::Vector3d f1_t = f1 - 1000 * from_dir;
        Eigen::Vector3d f2_t = f2 + 1000 * from_dir;
        p_from1 = {f1_t.x(), f1_t.y()};
        p_from2 = {f2_t.x(), f2_t.y()};
    }
...
bool get_cross_point(const Eigen::Vector3d &from, const Eigen::Vector3d &from_dir,
        const Eigen::Vector3d &to, const Eigen::Vector3d &to_dir, Eigen::Vector3d &ret, bool ignore_z) {
    Eigen::Vector3d from2 = from + from_dir;
    Eigen::Vector3d to2 = to + to_dir;
    return get_cross_point_by_point(from, from2, to, to2, ret, ignore_z, 3);
}


bool get_cross_point_by_point1(const Eigen::Vector3d &from1, const Eigen::Vector3d &from2,
        const Eigen::Vector3d &to1, const Eigen::Vector3d &to2, Eigen::Vector3d &ret, bool ignore_z) {
    double x;
	double y;
    bool x1_invalid = from1.x() - from2.x() == 0;
    bool x2_invalid = to1.x() - to2.x() == 0;
    bool y1_invalid = from1.y() - from2.y() == 0;
    bool y2_invalid = to1.y() - to2.y() == 0;

    if (x1_invalid && x2_invalid) {
        return false;
    }

    if (y1_invalid && y2_invalid) {
        return false;
    }
    ret.z() = 0;
    if (!x1_invalid && !x2_invalid && !y1_invalid && !y2_invalid) {
        double k = (from1.y() - from2.y()) / (from1.x() - from2.x());
        double k1 = (to1.y() - to2.y()) / (to1.x() - to2.x());

        if (k == k1) {
            return false;
        }
        double b = from1.y() - k * from1.x();
        double b1 = to1.y() - k1 * to1.x();

        x = (b1 - b) / (k - k1);
        y = k * x + b;
        ret.x() = x;
        ret.y() = y;
    } else if (x1_invalid && y2_invalid) {
        ret.x() = from1.x();
        ret.y() = to1.y();
    } else if (x2_invalid && y1_invalid) {
        ret.x() = to1.x();
        ret.y() = from1.y();
    } else {
        if (x1_invalid) {
...
//交点确保在AB 和CD上
bool get_cross_point_by_point2(const Eigen::Vector3d &from1, const Eigen::Vector3d &from2,
    ...
    ret = {cross_point.x(), cross_point.y(), 0};
    if (ignore_z) {
        ret.z() = 0;
    } else {
        // 计算 Z 坐标
...
bool get_cross_point_by_dir(const Eigen::Vector3d &from1, const Eigen::Vector3d &dir1,
        const Eigen::Vector3d &from2, const Eigen::Vector3d &to2, Eigen::Vector3d &ret, int mode, bool ignore_z) {
    bg_point p_from1(from1.x(), from1.y());
    bg_point p_to1(from1.x() + dir1.x(), from1.y() + dir1.y());  
    bg_point p_from2(from2.x(), from2.y());
    bg_point p_to2(to2.x(), to2.y());

    Eigen::Vector3d to1 = {from1.x() + dir1.x(), from1.y() + dir1.y(), 0};

    if (mode == 1) {
        Eigen::Vector3d from_dir = get_dir(to1, from1, ignore_z);
        Eigen::Vector3d p_from1_t = from1 - 1000 * from_dir;
        Eigen::Vector3d p_to1_t = to1 + 1000 * from_dir;
        p_from1 = {p_from1_t.x(), p_from1_t.y()};
        p_to1 = {p_to1_t.x(), p_to1_t.y()};
    }

    if (mode == 2) {
        Eigen::Vector3d to_dir = get_dir(to2, from2, ignore_z);
        Eigen::Vector3d p_from2_t = from2 - 1000 * to_dir;
        Eigen::Vector3d p_to2_t = to2 + 1000 * to_dir;
        p_from2 = {p_from2_t.x(), p_from2_t.y()};
        p_to2 = {p_to2_t.x(), p_to2_t.y()};
    }
    std::vector<bg_point> out;
    bool is_cross = bg::intersection(segment_t{p_from1, p_to1}, segment_t{p_from2, p_to2}, out);
    if (out.size() != 1) {
        return false;
    }
...
bool get_cross_point_with_polygon(
    ...
    const std::vector<Eigen::Vector3d> &polygon, std::vector<Eigen::Vector3d> &intersection_points, bool ignore_z, bool multi_points, double len) {

    int polygon_size = polygon.size(); 
    bool is_polygon_closed = (polygon[polygon_size - 1] == polygon[0]);
    ...
    for (int i = 0; i < polygon_size - 1; ++i) {  // 处理从 0 到 n-2 个顶点
        ...
        bool is_intersect = get_cross_point_by_point2(extended_from, extended_to, poly_from, poly_to, intersection_point, ignore_z);
        if (is_intersect) {
