...
int BevDataProcessor::init() {
    ...
    if (FLAGS_use_middle_enable) {
        ...
        LOG_INFO("bev_road_model json path[{}]", file_name);
...
int BevDataProcessor::download_match_info(std::vector<std::shared_ptr<LinkInfo>> & link_info_ptr,std::vector<LinkInfo*> & link_info)
{
    ...
    for(auto& zb_dir: zb_data_list){
        LOG_INFO("match info zb dir:{}", zb_dir);
...
int BevDataProcessor::download_ground_pcd(std::vector<std::shared_ptr<KeyPose>> &key_poses, 
        bool is_las) {
    if (is_las) {
        std::vector<std::string> file_list;
        fs::path path(utils::fmt("{}/../{}", FLAGS_base_dir, FLAGS_ground_las_dir));
        if (!fs::exists(path)) {
            return fsdmap::FAIL;
        }
        ...
        for (fs::recursive_directory_iterator iter(path); iter!= end_iter; iter++){
            try{
                if (!fs::is_directory(*iter) && iter->path().extension() == ".las") {
        ...
        // utils::glob_dir(path.string().c_str(), file_list);
        // LOG_INFO("path = {}", path.string());
        ...
        for (int i = 0; i < file_list.size(); i++){
            ...
            if (!ifs.is_open()){
                LOG_ERROR("download las false !!! path = {}", filename); 
                return fsdmap::FAIL;;
            }
            liblas::ReaderFactory f;
            liblas::Reader reader = f.CreateWithStream(ifs);
            liblas::Header header = reader.GetHeader();    //读取文件头
            int64_t pointsNum = header.GetPointRecordsCount();    //读取点数
            //初始化cloud
            int64_t start_index = key_pose->pcd->points.size();
            key_pose->pcd->points.resize(start_index + pointsNum);
            key_pose->pcd->width = key_pose->pcd->points.size();
            key_pose->pcd->height = 1;
            std::vector<uint8_t> pclassification(pointsNum);    //记录类别信息
            std::vector<liblas::Color> pcolor(pointsNum);    //记录颜色信息
            std::vector<uint16_t> pintencity(pointsNum);    //记录强度信息
            for (int i = 0; i < pointsNum; i++)    //这里用for比wihle安全
            {
                ...
                double pz = (double)p.GetZ();
                auto &pt = key_pose->pcd->points[start_index + i];
                pt.x = (px - _center_pos.x());
                pt.y = (py - _center_pos.y());
                pt.z = (pz - _center_pos.z());
                pt.label = p.GetClassification().GetClass();
        ...
        // pcl::io::savePCDFile<utils::CloudPoint>("./test.pcd", *key_pose->pcd);
    } else {
        std::vector<std::string> trail_list;
        boost::filesystem::path path(utils::fmt("{}/../{}", FLAGS_base_dir, FLAGS_ground_pcd_file));
        std::string pcd_file = path.string();
...
bool BevDataProcessor::search_nearest_pose(pcl::PointXYZI& searchPt)
{
...
int BevDataProcessor::download_lane_line(std::vector<std::shared_ptr<Feature>> &feature_vec) {
...
#if 0
    std::vector<std::string> zb_list;
    std::string bev_dir = FLAGS_bev_dir;
    std::string base_dir = utils::fmt("{}/*", FLAGS_base_dir);
    utils::glob_dir(base_dir.c_str(), zb_list);

    for(auto& zb_dir: zb_list){

        std::vector<std::string> trail_list;
        std::string lane_dir = utils::fmt("{}/{}/{}/*", zb_dir,bev_dir, FLAGS_mapping_line_dir);
        utils::glob_dir(lane_dir.c_str(), trail_list);
        ...
        for (int i = 0; i < trail_list.size(); ++i) {
            ...
            std::vector<std::string> file_list;
            utils::glob_dir_all(trail_dir.c_str(), file_list, true);

            for (auto json_file : file_list) {
                // LOG_ERROR("lane json file:{}",json_file)
                _thread_pool->schedule([&feature_vec, json_file, &trail_id,this](utils::ProcessBar *bar) {
                    ...
                    if(!ifs.is_open()){
                        LOG_ERROR("failed to lane line[path={}]", json_file);
                        return;
                    }
                    // LOG_ERROR("lane json file [path={}]", json_file);
                    nlohmann::json root;
                    try {
                        root = nlohmann::json::parse(ifs);
                    } catch (const std::exception& e) {
                        LOG_ERROR("failed to parse[{}, msg={}]", json_file, e.what());
                        return ;
                    }

                    int64_t tmstmp = int64_t(root["header"]["measurementTimestamp"].get<double>()*1e3);      
                    // double key_pose_timestamp =  double(tmstmp)/1e3;       
                    // LOG_INFO("feature_key_frame{}", frame_id);
                    std::string frame_id = get_frame_id_by_timestamp(tmstmp);
                    if (frame_id == "") {
                        LOG_ERROR("failed to match key frame[file={}]", json_file);
                        return;
                    }
                    ...
                    // LOG_ERROR("lane line {} {}", key_pose_timestamp, frame_id);

                    //parse json
                    auto &lane_line_list = root["laneMarkers"];

                    int64_t skip_num = 0;
                    
                    for (int i = 0; i < lane_line_list.size(); ++i) {
                        const auto &lane_line = lane_line_list[i];
                        // if (ins["task"].get<std::string>() != "bev_static") {
                        //     continue;
                        // }

                        std::string lane_id = utils::fmt("{}_{}", frame_id, i);
                        // int data_type = ins["attrs"]["type"].get<int>();
                        if (lane_line.contains("polyline")){
                            auto data_point = lane_line["polyline"];
                            ...
                            for (int j = 0; j < data_point.size(); ++j) {
                                ...
                                Eigen::Vector3d local_pos = {point["x"].get<double>(), point["y"].get<double>(), 0};
                                double dis = local_pos.norm();
                                if(dis > FLAGS_max_valid_line_dis){
                                    continue;
                                }                               
                                auto feature = std::make_shared<Feature>();
                                feature->line_id = lane_id;
                                feature->line_index = j;
                                feature->frame_id = frame_id;
                                feature->trail_id = trail_id;
                                // feature->raw_pos = {point["x"].get<double>(), point["y"].get<double>(), -2};
                                feature->raw_pos = {point["x"].get<double>(), point["y"].get<double>(), 0};
                                ...
                                feature->raw_pos = key_pose->r * feature->raw_pos;

                                alg::rotate_yaw(feature->raw_pos, feature->raw_pos, -90);

                                double lon,lat,h;
                                Eigen::Vector3d wgs;
                                ENU_TO_WGS(wgs.y(),wgs.x(),wgs.z(),key_pose->wgs.y(),key_pose->wgs.x(),key_pose->wgs.z(), 
                                feature->raw_pos.x(),feature->raw_pos.y(),feature->raw_pos.z());
                        

                                this->wgs2local(wgs, feature->pos);
                                // feature->pos = key_pose->r * feature->raw_pos;

                                // double radian = 90 * M_PI / 180;
                                // Eigen::AngleAxisd angle_axis(radian, Eigen::Vector3d(0, 0, 1));
                                // feature->pos = angle_axis.matrix() * feature->pos;

                                // feature->pos = feature->raw_pos + key_pose->pos;
                                ...
                                frame_feature_vec.push_back(feature);
    ...
    _thread_pool->wait(1, "download_lane_line");
    LOG_INFO("succ to download_lane_line[size={}]", feature_vec.size());
#else
    // std::vector<std::string> trail_list;
    ...
    if (!FLAGS_lane_line_use_cache) {
        ...
        std::string base_dir = utils::fmt("{}/../{}/*", FLAGS_base_dir, FLAGS_mapping_line_dir);
#if 0
        std::string base_dir2 = utils::fmt("{}/../{}", FLAGS_base_dir, FLAGS_mapping_line_dir);
        std::vector<std::string> target_link = {
            base_dir2+"/"+ "9130000056765",
            base_dir2+"/"+ "9100051857894",
        };
        int target_link_num = target_link.size();
#endif
        ...
        for (auto &trail_str : trail_list) {
            ...
            for (int i = 0; i < target_link_num; i++) {
                if (trail_str == target_link[i]) {
                    should_run = true;
                }
            }
            if (should_run == false) {
            ...
            std::string lane_file = utils::fmt("{}/laneBoundaryFromSeg/0_trjCloud.pcd", trail_str);
#else
            std::string lane_file = utils::fmt("{}/laneBoundaryFromSeg/*_trjCloud.pcd", trail_str);
#endif
            std::vector<std::string> tmp;
    ...
    for (int j = 0; j < global_pcd->points.size(); ++j) {
        ...
        bool has_found = search_nearest_pose(searchPt);
        if(has_found) {
        ...
        if (this->_data_type == "MMT_RC") {
            data_type = 5;
        } else if (this->_data_type == "BYD_BEV" || this->_data_type == "BYD_LIDAR_B") {
            data_type = 9;
        }
...
int BevDataProcessor::download_lane_line_mmt(std::vector<std::shared_ptr<Feature>> &feature_vec) {
    std::vector<std::string> zb_list;
    std::string base_dir = utils::fmt("{}/*/*", FLAGS_base_dir);
    utils::glob_dir(base_dir.c_str(), zb_list);
    std::vector<std::shared_ptr<Feature>> frame_feature_vec;

    for (const auto& zb_dir: zb_list) {
        if(!boost::filesystem::is_directory(zb_dir)){
            continue;
        }  

        std::string lane_dir = utils::fmt("{}/_ddld_landmark.json", zb_dir);
        if(!boost::filesystem::exists(lane_dir)){
            continue;
        }

        std::ifstream ifs(lane_dir);
        if(!ifs.is_open()){
            LOG_ERROR("failed to lane line[path={}]", lane_dir);
            continue;
        }
        ...
        try {
            ...
            LOG_ERROR("failed to parse[{}, msg={}]", lane_dir, e.what());
            continue;
        }

        if (!root.is_array()) {
            LOG_ERROR("Parsed root is not an array[file={}]", lane_dir);
            continue;
        }

        for (const auto& item : root) {
            if (!item.contains("meta") || !item["meta"].is_object() || 
                !item["meta"].contains("sensor_timestamp_us")) {
                LOG_ERROR("Invalid or missing metadata[file={}]", lane_dir);
                continue;
            }

            int64_t tmstmp = item["meta"]["sensor_timestamp_us"].get<int64_t>() / 1e3; 
            std::string frame_id = get_frame_id_by_timestamp(tmstmp);
            if (frame_id == "") {
                LOG_ERROR("failed to match key frame[file={}]", lane_dir);
                continue;
            }
         
            if (!item.contains("landmarks") || !item["landmarks"].is_object() || 
                !item["landmarks"].contains("lane_boundaries") || 
                !item["landmarks"]["lane_boundaries"].is_array()) {
                LOG_ERROR("Invalid or missing landmarks[file={}]", lane_dir);
                continue;
            }
            
            const auto &lane_boundaries = item["landmarks"]["lane_boundaries"];           
            for (size_t i = 0; i < lane_boundaries.size(); ++i) {
                const auto &lane_boundary  = lane_boundaries[i];
                std::string lane_id = utils::fmt("{}_{}", frame_id, i);

                if (!lane_boundary .contains("line_points") || !lane_boundary ["line_points"].is_array()) {
                    LOG_ERROR("line_points key doesn't exist or is not an array[file={}]", lane_dir);
                    continue;
                }

                if (lane_boundary["track_id"]["id"].is_null())
                {
                    continue;
                }
                
                const auto line_points = lane_boundary["line_points"];
                Feature* prev_feature = nullptr;

                for (size_t j = 0; j < line_points.size(); ++j) {
                    const auto &point = line_points[j];
                    Eigen::Vector3d local_pos = {point["x"].get<double>(), point["y"].get<double>(), 0};                   
                    if (local_pos.norm() > FLAGS_max_valid_line_dis){
                        continue;
                    }    
                    ...
                    feature->line_index = j;
                    feature->frame_id = frame_id;
                    feature->trail_id = lane_dir;
                    feature->raw_pos = local_pos;
                    auto key_pose = this->get_key_pose(frame_id);
                    if (!key_pose) {
                        LOG_ERROR("failed to retrieve key pose for frame_id={}", frame_id);
                        continue; 
                    }
                    feature->raw_pos = key_pose->r * feature->raw_pos;
                    // alg::rotate_yaw(feature->raw_pos, feature->raw_pos, -90);
...
int BevDataProcessor::download_site_boundary_line(std::vector<std::shared_ptr<Feature>> &feature_vec) {
    ...
    if (!FLAGS_boundary_line_use_cache) {
        ...
        for (int i = 0; i < contour_list.size(); ++i) {
            ...
            std::cout << "[download_site_boundary_line] index: " << i << " size: " << contour.size() << std::endl; 
        }
        ...
        for (int i = 0; i < lane_list.size(); ++i) {
            ...
            for (int j = 0; j < lane_pcd->points.size(); ++j) {
                auto &pt = lane_pcd->points[j];
                float mask = 5000 + i + (float)j / 10000;
                pt.intensity = mask;
...
int BevDataProcessor::download_boundary_line(std::vector<std::shared_ptr<Feature>> &feature_vec) {
    ...
    std::string line_dir = utils::fmt("{}/*", base_dir);
    utils::glob_dir(line_dir.c_str(), zb_list);

    for(auto& zb_dir: zb_list){
        std::vector<std::string> trail_list;

        std::string lane_dir = utils::fmt("{}/{}/{}/*", zb_dir,bev_dir, FLAGS_mapping_line_dir_boundary);
        utils::glob_dir(lane_dir.c_str(), trail_list);
        ...
        for (int i = 0; i < trail_list.size(); ++i) {
            ...
            for (auto json_file : file_list) {
                ...
                _thread_pool->schedule([&feature_vec, json_file, &trail_id,this](utils::ProcessBar *bar) {
                    ...
                    if(!ifs.is_open()){
                        LOG_ERROR("failed to boundary line[path={}]", json_file);
                        return;
                    }
                    // LOG_ERROR("lane json file [path={}]", json_file);
                    nlohmann::json root;
                    ...
                    //parse json
                    auto &boundary_line_list = root["roadEdges"];

                    int64_t skip_num = 0;
                    
                    for (int i = 0; i < boundary_line_list.size(); ++i) {
                        const auto &boundary_line = boundary_line_list[i];
                        // if (ins["task"].get<std::string>() != "bev_static") {
                        //     continue;
                        // }
                        ...
                        std::string lane_id = utils::fmt("{}_{}", frame_id, i);
                        // int data_type = ins["attrs"]["type"].get<int>();
                        if (boundary_line.contains("polyline")){
                            auto data_point = boundary_line["polyline"];
                            // if (data_score < FLAGS_bev_lable_min_score || data_score > FLAGS_bev_lable_max_score) {
                            //     skip_num += data_point.size();
                            //     continue;
                            // }
                            ...
                            for (int j = 0; j < data_point.size(); ++j) {
                                ...
                                // }
                                auto key_pose = this->get_key_pose(frame_id);
                                feature->raw_pos = key_pose->r * feature->raw_pos;  // 只将方向的转换到第一帧车辆坐标系

                                alg::rotate_yaw(feature->raw_pos, feature->raw_pos, -90);  // 逆时针旋转90度
    ...
    _thread_pool->wait(1, "download_boundary_line");
    LOG_INFO("succ to download_boundary_line[size={}]", feature_vec.size());
    ...
    if (!FLAGS_boundary_line_use_cache) {
        ...
        std::string base_dir2 = utils::fmt("{}/../{}", FLAGS_base_dir, FLAGS_mapping_line_dir);
        std::vector<std::string> target_link = {
            base_dir2+"/"+ "9100051857892",
            base_dir2+"/"+ "9100051857893",
            base_dir2+"/"+ "9100051857894",

            base_dir2+"/"+ "9130000056765",
            base_dir2+"/"+ "9130000056766",
            base_dir2+"/"+ "9500004596411",
            base_dir2+"/"+ "11006328237304",
            base_dir2+"/"+ "11004790401751",
            base_dir2+"/"+ "62059899",
            ...
            // base_dir2+"/"+ "9500002622044",
            // base_dir2+"/"+ "9000000680987",
            // base_dir2+"/"+ "9000000681047",
            // base_dir2+"/"+ "9100016530502",
            // base_dir2+"/"+ "9500409783312",
            // base_dir2+"/"+ "9500356564924",
            // base_dir2+"/"+ "9500014242677",
        };
        int target_link_num = target_link.size();
#endif
        ...
        for (auto &trail_str : trail_list) {
            ...
            // if(trail_str == target_link[0]) {
            //     lane_file = utils::fmt("{}/roadBoundaryFromSeg/2_trjCloud.pcd", trail_str);
            // }
            // std::string lane_file = utils::fmt("{}/roadBoundaryFromSeg/2_trjCloud.pcd", trail_str);
#else
            std::string lane_file = utils::fmt("{}/roadBoundaryFromSeg/*_trjCloud.pcd", trail_str);
#endif
            std::vector<std::string> tmp;
        ...
        for (int i = 0; i < lane_list.size(); ++i) {
            ...
            for (int j = 0; j < lane_pcd->points.size(); ++j) {
                ...
                obj_points.push_back(Eigen::Vector3d(pt.x, pt.y, pt.z));
            }
            // 判断该线段是否为边界线的点
...
#else        
    std::string version = "v4_89";
    std::string cache_file = utils::fmt("{}/{}", 
            FLAGS_base_dir, FLAGS_boundary_line_cache_file);

    utils::CloudPtr global_pcd(new utils::Cloud);
    if (!FLAGS_boundary_line_use_cache) {
        std::vector<std::string> trail_list;
        std::vector<std::string> boundary_list;
        std::string base_dir = utils::fmt("{}/../{}/roadboundary*.pcd",
                FLAGS_base_dir, FLAGS_mapping_line_dir_boundary);
        // utils::glob_dir(base_dir.c_str(), trail_list);
        utils::glob_dir(base_dir.c_str(), boundary_list);
        {
            std::vector<std::string> tmp;
            std::string diversion_dir = utils::fmt("{}/../{}/roadboundary*.pcd",
                FLAGS_base_dir, FLAGS_mapping_line_dir_diversion);
            utils::glob_dir(diversion_dir.c_str(), tmp);
            VEC_PUSH_ALL(boundary_list, tmp);
        }
        // for (auto &trail_str : trail_list) {
        //     if (!fs::is_directory(trail_str)) {
        //         continue;
        //     }
        //     std::string boundary_file = utils::fmt("{}/roadBoundaryFromSeg/*_trjCloud.pcd", trail_str);
        //     std::vector<std::string> tmp;
        //     utils::glob_dir(boundary_file.c_str(), tmp);
        //     VEC_PUSH_ALL(boundary_list, tmp);
        // }
        for (int i = 0; i < boundary_list.size(); ++i) {
            auto &boundary_str = boundary_list[i];
            utils::CloudPtr boundary_pcd(new utils::Cloud);
            if (pcl::io::loadPCDFile<utils::CloudPoint>(boundary_str.c_str(), *boundary_pcd) == -1) {
                LOG_ERROR("failed to read pcd[{}]", boundary_str);
                return fsdmap::FAIL;;
            }
            for (int j = 0; j < boundary_pcd->points.size(); ++j) {
                auto &pt = boundary_pcd->points[j];
                float mask = i + (float)j / 10000;
                pt.intensity = mask;
            }
            *global_pcd += *boundary_pcd;
        }
        if (global_pcd->size() > 0) {
            pcl::io::savePCDFile<utils::CloudPoint>(cache_file.c_str(), *global_pcd);
            LOG_INFO("cache boundary_line[file={}]", cache_file);
            utils::CloudPtr lane_pcd(new utils::Cloud);
            *lane_pcd = *global_pcd;
            for (int i = 0; i < lane_pcd->points.size(); ++i) {
                auto &pt = lane_pcd->points[i];
                pt.x += this->_center_pos.x();
            ...
            auto global_cache_file = utils::fmt("{}_global.pcd", cache_file);
            pcl::io::savePCDFile<utils::CloudPoint>(global_cache_file.c_str(), *lane_pcd);
            LOG_INFO("cache boundary_line[file={}]", global_cache_file);
        }
    } else {
        if (pcl::io::loadPCDFile<utils::CloudPoint>(cache_file.c_str(), *global_pcd) == -1) {
            LOG_ERROR("failed to read pcd[{}]", cache_file);
            return fsdmap::FAIL;;
        }
    }
    for (int j = 0; j < global_pcd->points.size(); ++j) {
        ...
        int64_t line_id = (int)pt.intensity;
        int64_t line_index = (int)(pt.intensity - line_id) * 10000;
        feature->line_id = utils::fmt("raw_boundary_{}", line_id);
        feature->line_index = line_index;
        feature->frame_id = "";
        feature->pos = {pt.x, pt.y, pt.z};
        feature->raw_pos = feature->pos;
...
int BevDataProcessor::download_boundary_line_mnt(std::vector<std::shared_ptr<Feature>> &feature_vec) {
    std::vector<std::string> trail_list;
    std::string base_dir;
    ...
    for (int i = 0; i < trail_list.size(); ++i) {
        ...
        std::string file_dir = utils::fmt("{}/{}", trail_dir, FLAGS_land_mark_file);

        boost::filesystem::path trail_path(trail_dir);
        std::string trail_id = trail_path.string();
        std::string tile_id = trail_path.parent_path().filename().string();
        // if (FLAGS_bev_trail_id != 0 && std::stol(trail_id) != FLAGS_bev_trail_id) {
        //     LOG_DEBUG("skip trail[{}]", trail_id);
        //     continue;
        ...
        for (auto &frame : root)
        {
            ...
            double frame_time_us = frame["meta"]["egopose_relevant_frame"]["timestamp_us"].get<double>();
            // std::string frame_time_ms = std::to_string(int64_t(frame_time_us * 1e-3));
            std::string frame_id_time = get_frame_id_by_timestamp(int64_t(frame_time_us * 1e-3));
            // std::string frame_id = get_frame_id(trail_id, frame_id_time);
            std::string frame_id = frame_id_time;

            if (frame_id == "")
            {
                LOG_ERROR("failed to match key frame[file={}]", file_dir);
                continue;
            }

            if (MAP_NOT_FIND(this->_key_map, frame_id))
            {
                LOG_DEBUG("failed to match key frame[file={}]", file_dir);
                continue;
            }
            ...
            // int instance_size = frame["landmarks"]["lane_centers"].size() +
            //                     frame["landmarks"]["lane_boundaries"].size() + frame["landmarks"]["road_boundaries"].size();
            int instance_index = 0;
        

            int64_t skip_num = 0;
            for(auto &lane_center : frame["landmarks"]["road_boundaries"]){
                double data_score = lane_center["score"].get<double>();
                auto data_point = lane_center["line_points"];
                int line_type = lane_center["type"];
                int data_type = convert_mnt_road_boundaries_type(line_type);
                ...
                std::string ins_id = utils::fmt("{}_{}", frame_id, instance_index);
...
int BevDataProcessor::download_intersection(std::vector<std::shared_ptr<Intersection>> &inter_list) {
    ...
    nlohmann::json root = nlohmann::json::parse(json_file);
    auto &json_inter_list = root["middle"]["sub_crosses"];
    
    for (int i = 0; i < json_inter_list.size(); ++i) {
        const auto &json_inter = json_inter_list[i];
        std::string inter_id = utils::fmt("{}", json_inter["sub_cross_id"].get<int64_t>());
        auto &bind_trails = json_inter["related_links"];
        auto tar_inter = std::make_shared<Intersection>();
        tar_inter->id = inter_id;
        auto &arr = json_inter["center"];
        Eigen::Vector3d wgs = {arr[0], arr[1], arr[2]};
        this->wgs2local(wgs, tar_inter->pos);
        ...
        // }
        inter_list.push_back(tar_inter);
        // 处理范围
...
//TODO : 待确实 读取的是什么，和junction的区别
int BevDataProcessor::download_intersection_mnt(std::vector<std::shared_ptr<Intersection>> &inter_list) {
    ...
    for (int i = 0; i < trail_list.size(); ++i) {
        ...
        for (auto &frame : root)
        {
            double frame_time_us = frame["meta"]["egopose_relevant_frame"]["timestamp_us"].get<double>();
            std::string frame_time_ms = std::to_string(int64_t(frame_time_us * 1e-3));
            std::string frame_id = get_frame_id(trail_id, frame_time_ms);

            if (MAP_NOT_FIND(this->_key_map, frame_id))
            {
                LOG_DEBUG("failed to match key frame[file={}]", frame_id);
                continue;
            }

            size_t aois_size = frame["landmarks"]["aois"].size();

            for (auto &aois : frame["landmarks"]["aois"])
            {
                Eigen::Vector3d polygon_center;
                size_t points_size = aois["polygon_points"].size();
                if(points_size < 1) continue;
                int act_size = 0;
                for (size_t i = 0; i < points_size; ++i)
                {
                    auto intersection_point = aois["polygon_points"][i];
                    if (intersection_point["x"].is_null() || intersection_point["y"].is_null() ||
                        intersection_point["z"].is_null())
                    {
                        continue;
                    }
                    polygon_center.x() += intersection_point["x"].get<double>();
                    polygon_center.y() += intersection_point["y"].get<double>();
                    polygon_center.z() += intersection_point["z"].get<double>();
                    act_size++;
                }
                polygon_center /= static_cast<double>(act_size); // pose to vehicle frame

                auto key_pose = this->get_key_pose(frame_id);
                // alg::rotate_yaw(feature->raw_pos, feature->pos, key_pose->yaw);
                Eigen::Vector3d polygon_center_utm;
                Eigen::Vector3d polygon_center_wgs;
                auto tar_inter = std::make_shared<Intersection>();

                polygon_center_utm = key_pose->r * polygon_center + key_pose->pos;
                utils::utm_2_wgs(_utm_zone, polygon_center_utm, polygon_center_wgs);
                this->wgs2local(polygon_center_wgs, tar_inter->pos);
                tar_inter->id = index;
                inter_list.push_back(tar_inter);
                index++;
            }
...
int BevDataProcessor::download_link(std::vector<std::shared_ptr<KeyPoseLine>> &link_list, 
...
#if 0
    std::string file_name = FLAGS_link_json_path;
    std::ifstream json_file;
    json_file.open(file_name);
    ...
    nlohmann::json root = nlohmann::json::parse(json_file);
    auto &json_link_list = root["links"];
    ...
    for (int i = 0; i < json_link_list.size(); ++i) {
        ...
        tar_link->id = link_id;
        // tar_link->link_direction = json_link["link_direction"].get<int>();
        // for (int j = 0; j < bind_trails.size(); ++j) {
        //     auto &track = bind_trails[j];
        //     tar_link->bind_trail_id.push_back(
        //             utils::fmt("{}_{}", track["tile_id"].get<int64_t>(), track["trail_id"].get<int64_t>()));
        ...
        boost::geometry::read_wkt(json_link["link_geom"], new_line); // link线的点信息
        ...
        for (auto &pt : new_line) {
            ...
            this->wgs2local(wgs, tar_pt->pos);  // wgs坐标系转换到第一帧车辆坐标系下
            link_point_list.push_back(tar_pt);
            tar_link->list.push_back(tar_pt.get());
            if (prev_poss != NULL) { // 计算前后连接关系
                prev_poss->next = tar_pt.get();
                tar_pt->prev = prev_poss;
                prev_poss->dir = alg::get_dir(tar_pt->pos, prev_poss->pos); // 计算从prev到cur的行驶方向
...
int BevDataProcessor::download_link_mnt(std::vector<std::shared_ptr<KeyPoseLine>> &link_list, 
    ...
    nlohmann::json root = nlohmann::json::parse(json_file);
    auto &json_link_list = root["features"];
    bool has_task_polygon = false;
    
    for (int i = 0; i < json_link_list.size(); ++i) {
        const auto &json_link = json_link_list[i];
        // std::string link_id = utils::fmt("{}", json_link["properties"]["id"].get<int64_t>());
        std::string link_id = utils::fmt("{}", json_link["properties"]["id"].get<std::string>());
        // auto &bind_trails = json_link["tracks"];
        auto tar_link = std::make_shared<KeyPoseLine>();
        tar_link->id = link_id;
        // tar_link->link_direction = json_link["link_direction"].get<int>();
        ...
        for(auto &elems : json_link["geometry"]["coordinates"]){
            for(auto &item : elems){
                double x = item[0].get<double>();
                double y = item[1].get<double>();
                wkt = wkt + std::to_string(x) + " " + std::to_string(y) + ","; 
            }
        }
        if(!wkt.empty()){
            wkt = wkt.substr(0, wkt.size()-1);
            wkt = wkt + ")";
        }
        GisPolyline new_line;
        boost::geometry::read_wkt(wkt, new_line);
        ...
        for (auto &pt : new_line) {
            ...
            Eigen::Vector3d wgs = {pt.get<0>(), pt.get<1>(), 0};
            // double lng_wgs, lat_wgs;
            // alg::gcj2Towgs(pt.get<0>(), pt.get<1>(), lng_wgs, lat_wgs);
            this->wgs2local(wgs, tar_pt->pos);
            link_point_list.push_back(tar_pt);
            tar_link->list.push_back(tar_pt.get());
            if (prev_poss != NULL) {
...
int BevDataProcessor::download_feature_mnt(std::vector<std::shared_ptr<Feature>> &feature_vec) {
    std::vector<std::string> trail_list;
    std::string base_dir;
    std::string bev_dir = FLAGS_base_dir;
    ...
    for (int i = 0; i < trail_list.size(); ++i) {
        ...
        for (auto &frame : root)
        {
            ...
            int instance_size = frame["landmarks"]["lane_centers"].size() +
                                frame["landmarks"]["lane_boundaries"].size() + frame["landmarks"]["road_boundaries"].size();
            int instance_index = 0;
            for(auto &lane_center : frame["landmarks"]["lane_centers"]){
                if(lane_center["available"] != 67){
                    continue;
                }
                double data_score = lane_center["score"].get<double>();
                auto data_point = lane_center["line_points"];
                int line_type = lane_center["lane_types"][0].get<int>();
                int data_type = convert_mnt_lane_center_type(line_type);
            ...
            for(auto &lane_center : frame["landmarks"]["lane_boundaries"]){
                if(lane_center["available"] != 127){
                    continue;
                }
                double data_score = lane_center["score"].get<double>();
                auto data_point = lane_center["line_points"];
                int line_type = lane_center["type"].get<int>();
                int data_type = convert_mnt_lane_boundaries_type(line_type);
            ...
            LOG_DEBUG("download feature[frame={}, ins_size={}, f_size={}, skip_num={}]",
                      frame_id, instance_size, frame_feature_vec.size(), skip_num);
...
int BevDataProcessor::download_object_mmt(
        std::vector<std::shared_ptr<RoadObjectInfo>> &raw_object_list) {
    ...
    for (const auto& zb_dir: zb_list) {
        ...
        std::string object_dir = utils::fmt("{}/_ddld_landmark.json", zb_dir);
        if(!boost::filesystem::exists(object_dir)){
            continue;
        }

        std::ifstream ifs(object_dir);
        if(!ifs.is_open()){
            LOG_ERROR("failed to lane line[path={}]", object_dir);
            continue;
        }
        ...
        try {
            ...
            LOG_ERROR("failed to parse[{}, msg={}]", object_dir, e.what());
            continue;
        }

        if (!root.is_array()) {
            LOG_ERROR("Parsed root is not an array[file={}]", object_dir);
            continue;
        }
        ...
        for (const auto& item : root) {      
            if (!item.contains("meta") || !item["meta"].is_object() || 
                !item["meta"].contains("sensor_timestamp_us")) {
                LOG_ERROR("Invalid or missing metadata[file={}]", object_dir);
                continue;
            }
            ...
            std::string frame_id = get_frame_id_by_timestamp(tmstmp);
            if (frame_id == "") {
                LOG_ERROR("failed to match key frame[file={}]", object_dir);
                continue;
            }
 
            if (!item.contains("landmarks") || !item["landmarks"].is_object() || 
                !item["landmarks"].contains("stop_lines") || !item["landmarks"]["stop_lines"].is_array() ||
                !item["landmarks"].contains("cross_walks") || !item["landmarks"]["cross_walks"].is_array() ||
                !item["landmarks"].contains("road_marks") || !item["landmarks"]["road_marks"].is_array()) {
                LOG_ERROR("Invalid or missing landmarks[file={}]", object_dir);
                continue;
            }

            // 获取停止线
            const auto &stop_lines = item["landmarks"]["stop_lines"];  
            int64_t stop_line_count = 0; // 初始化停止线计数器

            // 遍历所有停止线， 一个时间戳下的几条停止线，
            for (size_t i = 0; i < stop_lines.size(); ++i) {
                const auto &stop_line = stop_lines[i]; // 获取当前停止线数据
                int64_t line_id = 0; // 初始化线 ID
                
                // 检查该停止线是否具有有效的 track_id
                if (stop_line["track_id"]["id"].is_null()) {
                    continue; // 如果没有 track_id，跳过该停止线
                }
                line_id = stop_line["track_id"]["id"]; // 获取当前停止线的 track_id

                // 获取停止线的点数据
                const auto &line_points = stop_line["line_points"];
                
                // 检查停止线的点数据是否合法
                if (!line_points.is_array() || line_points.empty()) {
                    LOG_ERROR("Invalid or empty line points for stop line");
                    continue; 
                }

                // 使用 UMAP 存储每个线 ID 对应的 RoadObjectInfo
                UMAP<int64_t, std::shared_ptr<RoadObjectInfo>> ele_map;
                // std::vector<std::shared_ptr<RoadObjectPointInfo>> feature_container;
                // 遍历停止线上的所有点，每条2个点
                for (int j = 0; j < line_points.size(); ++j) {
                    auto &line_point = line_points[j]; // 获取当前点的数据
                    auto feature = std::make_shared<RoadObjectPointInfo>(); // 创建点的特征对象
                    
                    // 如果该线 ID 尚未存在于 ele_map 中，初始化该线的信息
                    if (MAP_NOT_FIND(ele_map, line_id)) {
                        ele_map[line_id] = std::make_shared<RoadObjectInfo>();
                        auto &obj = ele_map[line_id];
                        obj->pos = {0, 0, 0}; // 设置初始位置为原点
                        obj->ele_type = 6; // 设置元素类型为停止线
                        obj->score = stop_line["score"]; // 设置停止线的评分
                        obj->type = std::to_string(stop_line["type"].get<int>()); // 设置停止线的类型
                    }

                    auto &obj = ele_map[line_id]; // 获取对应的 RoadObjectInfo 对象
                    obj->list.push_back(feature); // 原始数据点集合

                    // 获取并设置当前点的位置数据
                    feature->raw_pos = {line_point.at("x").get<double>(), 
                                        line_point.at("y").get<double>(), 
                                        line_point.at("z").get<double>()};
                    
                    // 获取当前帧的关键位姿信息
                    auto key_pose = this->get_key_pose(frame_id);
                    if (!key_pose) {
                        LOG_ERROR("failed to retrieve key pose for frame_id={}", frame_id);
                        continue; // 如果无法获取关键位姿，跳过当前点
                    }
                    ...
                    // 将 ENU 坐标转换为 WGS 坐标
                    Eigen::Vector3d wgs;
                    ENU_TO_WGS(wgs.y(), wgs.x(), wgs.z(), 
                                key_pose->wgs.y(), key_pose->wgs.x(), key_pose->wgs.z(),
                                feature->raw_pos.x(), feature->raw_pos.y(), feature->raw_pos.z());
                    ...
                    // 设置当前点的类型、线 ID 和线索引
                    feature->type = stop_line["type"].get<int>();
                    feature->line_id = std::to_string(line_id);
                    feature->line_index = stop_line_count;

                    // 更新该停止线位置的总和 ，并且是做了坐标转化的，为下一步求中心点
                    obj->pos += feature->pos;
                    // feature_container.push_back(feature);

                    stop_line_count++; // 更新停止线计数器
                    // std::cout << "停止线"  << line_points.size() << std::endl; //都是2 
                }
                
  
                

                // 对每个线 ID 对应的 RoadObjectInfo 对象进行处理
                for (auto &it : ele_map) {
                    auto &obj = it.second;
                    obj->pos /= obj->list.size(); // 计算中心点，并且是做了坐标转化的

                    //停止线的长度
                    float length = (obj->list[0]->pos - obj->list[1]->pos).norm(); 
                    obj->length = length;    
                    // std::cout << "length_x"  << length_x << std::endl; 
            ...
            // 人行横道斑马线
            const auto &cross_walks = item["landmarks"]["cross_walks"];
            int64_t cross_walks_count = 0;

            // 遍历每个交叉口的人行道
            for (size_t i = 0; i < cross_walks.size(); ++i) {
                const auto &cross_walk = cross_walks[i];
                int64_t line_id = 0;

                // 检查人行横道是否具有有效的 track_id，如果没有，则跳过
                if (cross_walk["track_id"]["id"].is_null()) {
                    continue;
                }
                line_id = cross_walk["track_id"]["id"];

                // 获取人行横道的中心线点集合
                const auto &center_line_points = cross_walk["center_line_points"];

                // 检查中心线点是否有效，若无效则跳过
                if (!center_line_points.is_array() || center_line_points.empty()) {
                    LOG_ERROR("Invalid or empty center line points for cross walk");
                    continue;
                }

                // 创建一个映射，用于存储按 track_id 分类的人行横道对象
                UMAP<int64_t, std::shared_ptr<RoadObjectInfo>> ele_map;

                // 遍历每个中心线点
                for (int j = 0; j < center_line_points.size(); ++j) {
                    auto &center_line_point = center_line_points[j];

                    // 为每个中心线点创建一个新的特征对象
                    auto feature = std::make_shared<RoadObjectPointInfo>();

                    // 如果这个 track_id 对应的 RoadObjectInfo 还没有创建，创建一个新的
                    if (MAP_NOT_FIND(ele_map, line_id)) {
                        ele_map[line_id] = std::make_shared<RoadObjectInfo>();
                        auto &obj = ele_map[line_id];

                        // 初始化 RoadObjectInfo 的属性
                        obj->pos = {0, 0, 0}; // 初始化位置
                        obj->ele_type = 5;     // 设置元素类型为人行横道
                        obj->score = cross_walk["score"]; // 记录分数
                        obj->type = "0"; // 设置默认类型为 "0"
                    }

                    // 获取该 track_id 对应的 RoadObjectInfo 对象
                    auto &obj = ele_map[line_id];
                    ...
                    // 获取该点的原始坐标
                    feature->raw_pos = {center_line_point.at("x").get<double>(), center_line_point.at("y").get<double>(), center_line_point.at("z").get<double>()};
                    ...
                    // 为该特征指定唯一的 line_id 和 line_index
                    feature->line_id = std::to_string(line_id);
                    feature->line_index = cross_walks_count;
                ...
                // 遍历该 track_id 对应的所有 RoadObjectInfo 对象，进行进一步处理
                for (auto &it : ele_map) {
                    auto &obj = it.second;
                    ...
                    //长宽
                    float length = (obj->list[2]->pos - obj->list[3]->pos).norm(); 
                    float width = (obj->list[0]->pos - obj->list[1]->pos).norm(); //短边
                    obj->length = length;    
                    obj->width = width;    


                    // 判断对象是否在处理范围内，如果不在范围内则跳过
                    if (!is_in_processe_scope(obj->pos)) {
                        continue;
                    }
            ...
            //箭头
            const auto &road_marks = item["landmarks"]["road_marks"];   
            int64_t road_marks_count = 0;
            for (size_t i = 0; i < road_marks.size(); ++i) {
                const auto &road_mark  = road_marks[i];
                int64_t line_id = 0;
                if (road_mark["track_id"]["id"].is_null())
                {
                    continue;
                }
                line_id = road_mark["track_id"]["id"];
                const auto &types = road_mark["types"];
                std::ostringstream oss;
                //跳过导流带
                if(types.size()==1 && types[0]=="13"){
                    continue;
                }
                // 遍历 "types" 数组中的每个元素
                ...
                const auto &polygon_points = road_mark["polygon_points"];
                if (!polygon_points.is_array() || polygon_points.empty()) {
                    LOG_ERROR("Invalid or empty polygon points for road mark");
                    continue;
                }

                UMAP<int64_t, std::shared_ptr<RoadObjectInfo>> ele_map;
                for (int j = 0; j < polygon_points.size(); ++j) {
                    const auto &polygon_point  = polygon_points[j];
                    auto feature = std::make_shared<RoadObjectPointInfo>();
                    if (MAP_NOT_FIND(ele_map, line_id)) {
                        ele_map[line_id] = std::make_shared<RoadObjectInfo>();
                        auto &obj = ele_map[line_id];
                        obj->pos = {0, 0, 0};
                        obj->ele_type = 3; // 箭头
                        obj->score = road_mark["score"];
                        obj->type = oss.str();
                    }
                    auto &obj = ele_map[line_id];
                    obj->list.push_back(feature);
                    feature->raw_pos = {polygon_point.at("x").get<double>(), polygon_point.at("y").get<double>(), polygon_point.at("z").get<double>()};
                    auto key_pose = this->get_key_pose(frame_id);
                    if (!key_pose) {
                        LOG_ERROR("failed to retrieve key pose for frame_id={}", frame_id);
                        continue; 
                    ...
                    feature->type = 0;
                    feature->line_id = std::to_string(line_id);
                    feature->line_index = road_marks_count;
                    obj->pos += feature->pos;
                    road_marks_count++;
                }
                for (auto &it : ele_map) {
...
int BevDataProcessor::download_junction_mmt(std::vector<std::shared_ptr<JunctionInfo>> &raw_junction_list, 
        std::vector<std::shared_ptr<ImpassableAreaInfo>> &raw_area_list) {
...
std::vector<std::string> zb_list;
    ...
    for (const auto& zb_dir: zb_list) {
        ...
        std::string junction_dir = utils::fmt("{}/_ddld_landmark.json", zb_dir);
        if(!boost::filesystem::exists(junction_dir)){
            continue;
        }

        std::ifstream ifs(junction_dir);
        if(!ifs.is_open()){
            LOG_ERROR("failed to lane line[path={}]", junction_dir);
            continue;
        }
        ...
        try {
            ...
            LOG_ERROR("failed to parse[{}, msg={}]", junction_dir, e.what());
            continue;
        }

        if (!root.is_array()) {
            LOG_ERROR("Parsed root is not an array[file={}]", junction_dir);
            continue;
        }
        ...
        for (const auto& item : root) {  
            count++;     
            if (!item.contains("landmarks") || !item["landmarks"].is_object() || 
                !item["landmarks"].contains("aois") || !item["landmarks"]["aois"].is_array()) {
                LOG_ERROR("Invalid or missing landmarks[file={}]", junction_dir);
                continue;
            }
            ...
            std::string frame_id = get_frame_id_by_timestamp(tmstmp);
            if (frame_id == "") {
                LOG_ERROR("failed to match key frame[file={}]", junction_dir);
                continue;
            }
            
            const auto &lu_kous = item["landmarks"]["aois"];  
            for (size_t i = 0; i < lu_kous.size(); ++i) {
                const auto &lu_kou  = lu_kous[i];

                auto obj = std::make_shared<JunctionInfo>(); 
                if (lu_kou["score"].is_null())
                {
                    continue;
                }
                obj->ele_type = 0; 
                // obj->score = lu_kou["score"]; //路口点没有 score 变量
                const auto &polygon_points = lu_kou["polygon_points"];
                if (!polygon_points.is_array() || polygon_points.empty()) {
                    LOG_ERROR("Invalid or empty line points for aois");
                    continue;
                }
                //aois没有track_id， 所以不做MAP_NOT_FIND判断
                for (size_t j=0; j < polygon_points.size(); ++j) {
                    const auto &polygon_point  = polygon_points[j];
                    auto feature = std::make_shared<RoadObjectPointInfo>();
                    obj->point_info.push_back(feature);  // add
                    obj->lukou_poly_pts.push_back(feature->raw_pos); // fix
                    feature->raw_pos = {polygon_point.at("x").get<double>(), 
                                    polygon_point.at("y").get<double>(), 
                                    polygon_point.at("z").get<double>()}; //fix
                    obj->pos = {0, 0, 0};
                    ...
                    feature->type = lu_kou["type"].get<int>();
                    feature->line_id = utils::fmt("raw_line_{}", i);
                    feature->line_index = j;
                    // std::cout << "lukou_poly_pts:" << obj->lukou_poly_pts.size() << "pos" <<feature->pos << "\n" << std::endl;
                    obj->pos += feature->pos;
                }

                if (!obj->point_info.empty()) {
                    obj->pos /= obj->point_info.size();
                    if (!is_in_processe_scope(obj->pos)) {
                        continue;
                    }
...
void BevDataProcessor::WGS_TO_ENU(double lat, double lon, double h, double lat0, double lon0, double h0, double &enu_x,double &enu_y,double &enu_z)
{
...
void BevDataProcessor::ENU_TO_WGS(double &lat, double &lon, double &h, double lat0, double lon0, double h0, double xEast,double yNorth,double zUp ){
	double a, b, f, e_sq, pi;
    pi = 3.14159265359;
	a = 6378137;
	b = 6356752.3142;
...
int BevDataProcessor::init_ele_type() {
    ...
    }
    {
        ...
        auto &type_map = ele_type_map["BYD_LIDAR_B"];
    ...
    {
        this->ele_type_map["v4_89"] = UMAP<int, std::tuple<ELEMENT_TYPE, int, int, cv::Scalar>>();
        auto &type_map = ele_type_map["v4_89"];
        type_map[0] = {ELEMENT_LANE_LINE, 0, 0, {255, 255, 255}};//车道线，UNKNOWN_COLOR，UNKNOWN
        type_map[1] = {ELEMENT_LANE_LINE, 0, 1, {255, 255, 255}};//车道线，UNKNOWN_COLOR，solid
        type_map[2] = {ELEMENT_LANE_LINE, 0, 2, {255, 255, 255}};//车道线，UNKNOWN_COLOR，dashed
        type_map[3] = {ELEMENT_LANE_LINE, 0, 3, {255, 255, 255}};//车道线，UNKNOWN_COLOR，SHORT_DASHED
        type_map[4] = {ELEMENT_LANE_LINE, 0, 4, {0, 255, 255}};//车道线，UNKNOWN_COLOR，double_solid
        type_map[5] = {ELEMENT_LANE_LINE, 0, 5, {0, 255, 255}};//车道线，UNKNOWN_COLOR，double_dashed
        type_map[6] = {ELEMENT_LANE_LINE, 0, 6, {0, 255, 255}};//车道线，UNKNOWN_COLOR，left_solid_right_dashed
        type_map[7] = {ELEMENT_LANE_LINE, 0, 7, {0, 255, 255}};//车道线，UNKNOWN_COLOR，left_dashed_right_solid
        type_map[8] = {ELEMENT_LANE_LINE, 1, 0, {255, 255, 255}};//车道线，白色线类，UNKNOWN
        type_map[9] = {ELEMENT_LANE_LINE, 1, 1, {255, 255, 255}};//车道线，白色线类，solid
        type_map[10] = {ELEMENT_LANE_LINE, 1, 2, {255, 255, 255}};//车道线，白色线类，dashed
        type_map[11] = {ELEMENT_LANE_LINE, 1, 3, {255, 255, 255}};//车道线，白色线类，SHORT_DASHED
        type_map[12] = {ELEMENT_LANE_LINE, 1, 4, {0, 255, 255}};//车道线，白色线类，double_solid
        type_map[13] = {ELEMENT_LANE_LINE, 1, 5, {0, 255, 255}};//车道线，白色线类，double_dashed
        type_map[14] = {ELEMENT_LANE_LINE, 1, 6, {0, 255, 255}};//车道线，白色线类，left_solid_right_dashed
        type_map[15] = {ELEMENT_LANE_LINE, 1, 7, {0, 255, 255}};//车道线，白色线类，left_dashed_right_solid
        type_map[16] = {ELEMENT_LANE_LINE, 2, 0, {255, 255, 0}};//车道线，黄色线类，UNKNOWN
        type_map[17] = {ELEMENT_LANE_LINE, 2, 1, {255, 255, 0}};//车道线，黄色线类，solid
        type_map[18] = {ELEMENT_LANE_LINE, 2, 2, {255, 255, 0}};//车道线，黄色线类，dashed
        type_map[19] = {ELEMENT_LANE_LINE, 2, 3, {255, 128, 100}};//车道线，黄色线类，SHORT_DASHED
        type_map[20] = {ELEMENT_LANE_LINE, 2, 4, {255, 128, 100}};//车道线，黄色线类，double_solid
        type_map[21] = {ELEMENT_LANE_LINE, 2, 5, {255, 128, 100}};//车道线，黄色线类，double_dashed
        type_map[22] = {ELEMENT_LANE_LINE, 2, 6, {255, 128, 100}};//车道线，黄色线类，left_solid_right_dashed
        type_map[23] = {ELEMENT_LANE_LINE, 2, 7, {255, 128, 100}};//车道线，黄色线类，left_dashed_right_solid
        // type, road_boundary, lane_boundary
        // 40 41 47 50 52 53 54
        type_map[24] = {ELEMENT_BARRIER, 0, 0, {152, 99, 60}};//道路边界，UNKNOWN
        type_map[25] = {ELEMENT_CURB,    2, 0, {81, 89, 240}};//道路边界，路缘石
        type_map[26] = {ELEMENT_BARRIER, 6, 0, {152, 99, 60}};//道路边界，栅栏
        type_map[27] = {ELEMENT_BARRIER, 7, 0, {152, 99, 60}};//道路边界，保护墙
        type_map[28] = {ELEMENT_BARRIER, 9, 0, {152, 99, 60}};//道路边界，自然边界,铺设未铺设边界
        type_map[29] = {ELEMENT_BARRIER, 10, 0, {152, 99, 60}};//道路边界，沟渠
        type_map[30] = {ELEMENT_BARRIER, 11, 0, {152, 99, 60}};//道路边界，离散型障碍物（包括短柱:可表达石墩\短柱等无法穿越的障碍物）
        type_map[31] = {ELEMENT_OBJECT,  8, 1, {255, 0, 0}};//停止线
        type_map[32] = {ELEMENT_OBJECT,  10, 1,{255, 0, 0}};// 人行道区域
        type_map[33] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};// 不可通行区域，polygon_unknown_kind
        type_map[34] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};// 不可通行区域，polygon_flowerbed(花坛)
        type_map[35] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};// 不可通行区域，polygon_sentry_box(岗亭)
        type_map[36] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};// 不可通行区域，polygon_physical_safe_island（安全岛）
        type_map[37] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};// 不可通行区域，polygon_linear_safe_island（安全岛）
        type_map[38] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};// 不可通行区域
        type_map[39] = {ELEMENT_JUNCTION, 1, 0, {100, 255, 100}};// 路口区域
        // 上下行分割
        type_map[40] = {ELEMENT_BARRIER, 100, 0, {255, 0, 0}};
        // 横向打断线
        type_map[41] = {ELEMENT_OBJECT, 100, 0, {100, 255, 100}};
        // 路口引导线
        type_map[42] = {ELEMENT_VIRTUAL_LANE_LINE, 12, 10, {0, 255, 0}};// virtual_lane_center
        // 车道中心线
        type_map[43] = {ELEMENT_LANE_CENTER, 3, 0, {0, 255, 0}};// lane_center，混合车道，unknown
        type_map[44] = {ELEMENT_LANE_CENTER, 3, 2, {0, 255, 0}};// lane_center，混合车道，普通车道
        type_map[45] = {ELEMENT_LANE_CENTER, 3, 3, {0, 255, 0}};// lane_center，混合车道，加速车道
        type_map[46] = {ELEMENT_LANE_CENTER, 3, 4, {0, 255, 0}};// lane_center，混合车道，减速车道
        type_map[47] = {ELEMENT_LANE_CENTER, 3, 17, {0, 255, 0}};// lane_center，混合车道，避险车道
        type_map[48] = {ELEMENT_LANE_CENTER, 3, 12, {0, 255, 0}};// lane_center，混合车道，应急车道
        type_map[49] = {ELEMENT_LANE_CENTER, 3, 5, {0, 255, 0}};// lane_center，混合车道，加减速车道
        type_map[50] = {ELEMENT_LANE_CENTER, 3, 12, {0, 255, 0}};// lane_center，混合车道
        type_map[51] = {ELEMENT_LANE_CENTER, 3, 6, {0, 255, 0}};// lane_center，混合车道，HOV车道
        type_map[52] = {ELEMENT_LANE_CENTER, 3, 14, {0, 255, 0}};// lane_center，混合车道，自行车道
        type_map[53] = {ELEMENT_LANE_CENTER, 3, 12, {0, 255, 0}};// lane_center，混合车道
        // 道路中心线
        type_map[54] = {ELEMENT_ROAD_CENTER, 3, 12, {0, 255, 0}};

        type_map[55] = {ELEMENT_OBJECT, 7, 0, {255, 255, 0}};// 箭头区域
        type_map[56] = {ELEMENT_OBJECT, 7, 1, {255, 255, 0}};// 箭头区域
        type_map[57] = {ELEMENT_OBJECT, 7, 2, {255, 255, 0}};// 箭头区域
        type_map[58] = {ELEMENT_OBJECT, 7, 3, {255, 255, 0}};// 箭头区域
        type_map[59] = {ELEMENT_OBJECT, 7, 4, {255, 255, 0}};// 箭头区域
        type_map[60] = {ELEMENT_OBJECT, 7, 5, {255, 255, 0}};// 箭头区域
        type_map[61] = {ELEMENT_OBJECT, 7, 6, {255, 255, 0}};// 箭头区域
        type_map[62] = {ELEMENT_OBJECT, 7, 7, {255, 255, 0}};// 箭头区域
        type_map[63] = {ELEMENT_OBJECT, 7, 8, {255, 255, 0}};// 箭头区域
        type_map[64] = {ELEMENT_OBJECT, 7, 9, {255, 255, 0}};// 箭头区域
        type_map[65] = {ELEMENT_OBJECT, 7, 10, {255, 255, 0}};// 箭头区域
        type_map[66] = {ELEMENT_OBJECT, 7, 11, {255, 255, 0}};// 箭头区域
        type_map[67] = {ELEMENT_OBJECT, 0, 12, {255, 255, 0}};
        type_map[68] = {ELEMENT_OBJECT, 0, 13, {255, 255, 0}};
        type_map[69] = {ELEMENT_OBJECT, 0, 14, {255, 255, 0}};
        type_map[70] = {ELEMENT_OBJECT, 7, 15, {255, 255, 0}};// 箭头区域
        type_map[71] = {ELEMENT_OBJECT, 7, 16, {255, 255, 0}};// 箭头区域
        type_map[72] = {ELEMENT_OBJECT, 7, 17, {255, 255, 0}};// 箭头区域
        type_map[73] = {ELEMENT_OBJECT, 7, 18, {255, 255, 0}};// 箭头区域
        type_map[74] = {ELEMENT_OBJECT, 7, 19, {255, 255, 0}};// 箭头区域
        type_map[75] = {ELEMENT_OBJECT, 7, 20, {255, 255, 0}};// 箭头区域
        type_map[76] = {ELEMENT_OBJECT, 0, 21, {255, 255, 0}};
        type_map[77] = {ELEMENT_OBJECT, 0, 22, {255, 255, 0}};
        type_map[78] = {ELEMENT_OBJECT, 0, 23, {255, 255, 0}};
        type_map[79] = {ELEMENT_OBJECT, 0, 24, {255, 255, 0}};
        type_map[80] = {ELEMENT_OBJECT, 0, 25, {255, 255, 0}};
        type_map[81] = {ELEMENT_OBJECT, 0, 27, {255, 255, 0}};
        type_map[82] = {ELEMENT_TRAFFICLIGHT, 13, 0, {255, 255, 0}};// trafficlight_NONE
        type_map[83] = {ELEMENT_TRAFFICLIGHT, 13, 1, {255, 255, 0}};// trafficlight_ROUND
        type_map[84] = {ELEMENT_TRAFFICLIGHT, 13, 2, {255, 255, 0}};// trafficlight_CROSS
        type_map[85] = {ELEMENT_TRAFFICLIGHT, 13, 3, {255, 255, 0}};// trafficlight_PEDESTRIAN
        type_map[86] = {ELEMENT_TRAFFICLIGHT, 13, 4, {255, 255, 0}};// trafficlight_BICYCLE
        type_map[87] = {ELEMENT_TRAFFICLIGHT, 13, 5, {255, 255, 0}};// trafficlight_ARROW
        type_map[88] = {ELEMENT_TRAFFICLIGHT, 13, 6, {255, 255, 0}};// trafficlight_TIME
    }
    {
        this->ele_type_map["v3_66"] = UMAP<int, std::tuple<ELEMENT_TYPE, int, int, cv::Scalar>>();
        auto &type_map = ele_type_map["v3_66"];
        ...
        type_map[3] = {ELEMENT_LANE_LINE, 0, 4, {0, 255, 255}};
        type_map[4] = {ELEMENT_LANE_LINE, 0, 5, {0, 255, 255}};
        type_map[5] = {ELEMENT_LANE_LINE, 0, 7, {0, 255, 255}};
        type_map[6] = {ELEMENT_LANE_LINE, 0, 6, {0, 255, 255}};
        type_map[7] = {ELEMENT_LANE_LINE, 1, 0, {255, 255, 255}};
        type_map[8] = {ELEMENT_LANE_LINE, 1, 1, {255, 255, 255}};
        type_map[9] = {ELEMENT_LANE_LINE, 1, 2, {255, 255, 255}};
        type_map[10] = {ELEMENT_LANE_LINE, 1, 4, {0, 255, 255}};
        type_map[11] = {ELEMENT_LANE_LINE, 1, 5, {0, 255, 255}};
        type_map[12] = {ELEMENT_LANE_LINE, 1, 7, {0, 255, 255}};
        type_map[13] = {ELEMENT_LANE_LINE, 1, 6, {0, 255, 255}};
        type_map[14] = {ELEMENT_LANE_LINE, 2, 0, {255, 255, 0}};
        type_map[15] = {ELEMENT_LANE_LINE, 2, 1, {255, 255, 0}};
        type_map[16] = {ELEMENT_LANE_LINE, 2, 2, {255, 255, 0}};
        type_map[17] = {ELEMENT_LANE_LINE, 2, 4, {255, 128, 100}};
        type_map[18] = {ELEMENT_LANE_LINE, 2, 5, {255, 128, 100}};
        type_map[19] = {ELEMENT_LANE_LINE, 2, 7, {255, 128, 100}};
        type_map[20] = {ELEMENT_LANE_LINE, 2, 6, {255, 128, 100}};
        // type, road_boundary, lane_boundary
        type_map[21] = {ELEMENT_BARRIER, 0, 0, {152, 99, 60}};
        type_map[22] = {ELEMENT_CURB,    2, 0, {81, 89, 240}};
        type_map[23] = {ELEMENT_BARRIER, 6, 0, {152, 99, 60}};
        type_map[24] = {ELEMENT_BARRIER, 7, 0, {152, 99, 60}};
        type_map[25] = {ELEMENT_BARRIER, 9, 0, {152, 99, 60}};
        type_map[26] = {ELEMENT_BARRIER, 10, 0, {152, 99, 60}};
        type_map[27] = {ELEMENT_BARRIER, 11, 0, {152, 99, 60}};
        type_map[28] = {ELEMENT_OBJECT,  8, 1, {255, 0, 0}};
        type_map[29] = {ELEMENT_OBJECT,  10, 1,{255, 0, 0}};
        type_map[30] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};
        type_map[31] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};
        type_map[32] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};
        type_map[33] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};
        type_map[34] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};
        type_map[35] = {ELEMENT_OBJECT,  0, 0, {255, 255, 255}};
        type_map[36] = {ELEMENT_LANE_CENTER, 3, 0, {0, 255, 0}};
        type_map[37] = {ELEMENT_JUNCTION, 1, 0, {100, 255, 100}};
        type_map[38] = {ELEMENT_VIRTUAL_LANE_LINE, 12, 10, {0, 255, 0}};
        type_map[39] = {ELEMENT_OBJECT, 7, 0, {255, 255, 0}};
        type_map[40] = {ELEMENT_OBJECT, 7, 1, {255, 255, 0}};
        type_map[41] = {ELEMENT_OBJECT, 7, 2, {255, 255, 0}};
        type_map[42] = {ELEMENT_OBJECT, 7, 3, {255, 255, 0}};
        type_map[43] = {ELEMENT_OBJECT, 7, 4, {255, 255, 0}};
        type_map[44] = {ELEMENT_OBJECT, 7, 5, {255, 255, 0}};
        type_map[45] = {ELEMENT_OBJECT, 7, 6, {255, 255, 0}};
        type_map[46] = {ELEMENT_OBJECT, 7, 7, {255, 255, 0}};
        type_map[47] = {ELEMENT_OBJECT, 7, 8, {255, 255, 0}};
        type_map[48] = {ELEMENT_OBJECT, 7, 9, {255, 255, 0}};
        type_map[49] = {ELEMENT_OBJECT, 7, 10, {255, 255, 0}};
        type_map[50] = {ELEMENT_OBJECT, 7, 11, {255, 255, 0}};
        type_map[51] = {ELEMENT_OBJECT, 7, 12, {255, 255, 0}};
        type_map[52] = {ELEMENT_OBJECT, 7, 13, {255, 255, 0}};
        type_map[53] = {ELEMENT_OBJECT, 7, 14, {255, 255, 0}};
        type_map[54] = {ELEMENT_OBJECT, 7, 15, {255, 255, 0}};
        type_map[55] = {ELEMENT_OBJECT, 7, 16, {255, 255, 0}};
        type_map[56] = {ELEMENT_OBJECT, 7, 17, {255, 255, 0}};
        type_map[57] = {ELEMENT_OBJECT, 7, 18, {255, 255, 0}};
        type_map[58] = {ELEMENT_OBJECT, 7, 19, {255, 255, 0}};
        type_map[59] = {ELEMENT_OBJECT, 7, 20, {255, 255, 0}};
        type_map[60] = {ELEMENT_OBJECT, 7, 21, {255, 255, 0}};
        type_map[61] = {ELEMENT_OBJECT, 7, 22, {255, 255, 0}};
        type_map[62] = {ELEMENT_OBJECT, 7, 23, {255, 255, 0}};
        type_map[63] = {ELEMENT_OBJECT, 7, 24, {255, 255, 0}};
        type_map[64] = {ELEMENT_OBJECT, 7, 25, {255, 255, 0}};
        type_map[65] = {ELEMENT_OBJECT, 7, 27, {255, 255, 0}};
    }
    {
        this->ele_type_map["v2_65"] = UMAP<int, std::tuple<ELEMENT_TYPE, int, int, cv::Scalar>>();
        auto &type_map = ele_type_map["v2_65"];
        type_map[0] = {ELEMENT_LANE_LINE, 0, 0, {255, 255, 255}};
        type_map[1] = {ELEMENT_LANE_LINE, 0, 1, {255, 255, 255}};
        type_map[2] = {ELEMENT_LANE_LINE, 0, 2, {255, 255, 255}};
        type_map[3] = {ELEMENT_LANE_LINE, 0, 4, {0, 255, 255}};
        ...
        type_map[36] = {ELEMENT_LANE_CENTER, 3, 0, {0, 255, 0}};
        type_map[37] = {ELEMENT_JUNCTION, 1, 0, {100, 255, 100}};
        type_map[38] = {ELEMENT_OBJECT, 7, 0, {255, 255, 0}};
        type_map[39] = {ELEMENT_OBJECT, 7, 1, {255, 255, 0}};
        type_map[40] = {ELEMENT_OBJECT, 7, 2, {255, 255, 0}};
        type_map[41] = {ELEMENT_OBJECT, 7, 3, {255, 255, 0}};
        type_map[42] = {ELEMENT_OBJECT, 7, 4, {255, 255, 0}};
        type_map[43] = {ELEMENT_OBJECT, 7, 5, {255, 255, 0}};
        type_map[44] = {ELEMENT_OBJECT, 7, 6, {255, 255, 0}};
        type_map[45] = {ELEMENT_OBJECT, 7, 7, {255, 255, 0}};
        type_map[46] = {ELEMENT_OBJECT, 7, 8, {255, 255, 0}};
        type_map[47] = {ELEMENT_OBJECT, 7, 9, {255, 255, 0}};
        type_map[48] = {ELEMENT_OBJECT, 7, 10, {255, 255, 0}};
        type_map[49] = {ELEMENT_OBJECT, 7, 11, {255, 255, 0}};
        type_map[50] = {ELEMENT_OBJECT, 7, 12, {255, 255, 0}};
        type_map[51] = {ELEMENT_OBJECT, 7, 13, {255, 255, 0}};
        type_map[52] = {ELEMENT_OBJECT, 7, 14, {255, 255, 0}};
        type_map[53] = {ELEMENT_OBJECT, 7, 15, {255, 255, 0}};
        type_map[54] = {ELEMENT_OBJECT, 7, 16, {255, 255, 0}};
        type_map[55] = {ELEMENT_OBJECT, 7, 17, {255, 255, 0}};
        type_map[56] = {ELEMENT_OBJECT, 7, 18, {255, 255, 0}};
        type_map[57] = {ELEMENT_OBJECT, 7, 19, {255, 255, 0}};
        type_map[58] = {ELEMENT_OBJECT, 7, 20, {255, 255, 0}};
        type_map[59] = {ELEMENT_OBJECT, 7, 21, {255, 255, 0}};
        type_map[60] = {ELEMENT_OBJECT, 7, 22, {255, 255, 0}};
        type_map[61] = {ELEMENT_OBJECT, 7, 23, {255, 255, 0}};
        type_map[62] = {ELEMENT_OBJECT, 7, 24, {255, 255, 0}};
        type_map[63] = {ELEMENT_OBJECT, 7, 25, {255, 255, 0}};
        type_map[64] = {ELEMENT_OBJECT, 7, 27, {255, 255, 0}};
    }
    {
        this->ele_type_map["v1_37"] = UMAP<int, std::tuple<ELEMENT_TYPE, int, int, cv::Scalar>>();
        auto &type_map = ele_type_map["v1_37"];
        type_map[0] = {ELEMENT_LANE_LINE, 0, 0, {255, 255, 255}};
        type_map[1] = {ELEMENT_LANE_LINE, 0, 1, {255, 255, 255}};
        type_map[2] = {ELEMENT_LANE_LINE, 0, 2, {255, 255, 255}};
        type_map[3] = {ELEMENT_LANE_LINE, 0, 4, {0, 255, 255}};
...
void BevDataProcessor::init_attr_map() {
    {
        ...
        auto &attr_map = this->attr_map["mmt_to_std"];
        attr_map[std::make_tuple("lane_center", "type", 0)] = 0; // 未知
        attr_map[std::make_tuple("lane_center", "type", 1)] = 1; // 常规
        attr_map[std::make_tuple("lane_center", "type", 5)] = 99; // 应急车道
        attr_map[std::make_tuple("lane_center", "type", 6)] = 99; // 公交车道
        attr_map[std::make_tuple("lane_center", "type", 15)] = 99; // 非机动车道
        attr_map[std::make_tuple("lane_center", "type", 20)] = 99; // 收费站
        attr_map[std::make_tuple("lane_center", "type", 28)] = 99; // 可变车道
        attr_map[std::make_tuple("lane_center", "type", 32)] = 99; // 潮汐车道
        attr_map[std::make_tuple("lane_center", "type", 71)] = 99; // 左右转或掉头代转区
        attr_map[std::make_tuple("lane_center", "type", 73)] = 99; // 不完整车道
        attr_map[std::make_tuple("lane_center", "type", 74)] = 99; // block(byd)
        attr_map[std::make_tuple("lane_boundary", "type", 0)] = 0; // 未知        
        attr_map[std::make_tuple("lane_boundary", "type", 1)] = 3; // 虚线
        attr_map[std::make_tuple("lane_boundary", "type", 2)] = 2; // 实线    
        // 新增   
        attr_map[std::make_tuple("lane_boundary", "type", 3)] = 4; // 双实线
        attr_map[std::make_tuple("lane_boundary", "type", 4)] = 5; // 双虚线
        attr_map[std::make_tuple("lane_boundary", "type", 5)] = 6; // 左实右虚线
        attr_map[std::make_tuple("lane_boundary", "type", 6)] = 7; // 左虚右实线
        
        attr_map[std::make_tuple("lane_boundary", "color", 0)] = 1; // 未知
        attr_map[std::make_tuple("lane_boundary", "color", 1)] = 2; // 白色
        attr_map[std::make_tuple("lane_boundary", "color", 2)] = 3; // 黄色
        attr_map[std::make_tuple("road_boundary", "type", 0)] = 0; // 未知        
        attr_map[std::make_tuple("road_boundary", "type", 2)] = 1; // 普通     
        attr_map[std::make_tuple("road_boundary", "type", 4)] = 7; // 水马
        attr_map[std::make_tuple("road_boundary", "type", 5)] = 8; // 锥桶
        attr_map[std::make_tuple("road_boundary", "type", 20)] = 99; // 锥桶水马混合/施工牌/防撞桶
        attr_map[std::make_tuple("stop_line", "type", 0)] = 0; // 未知
        attr_map[std::make_tuple("stop_line", "type", 1)] = 1; // 普通停止线
        attr_map[std::make_tuple("stop_line", "type", 2)] = 1; // 左转待行区停止线
        attr_map[std::make_tuple("stop_line", "type", 3)] = 1; // 掉头待行区停止线
        attr_map[std::make_tuple("stop_line", "type", 4)] = 1; // 右转待行区停止线
        attr_map[std::make_tuple("stop_line", "type", 5)] = 1; // 直行待行区停止线
        attr_map[std::make_tuple("road_mark", "type", 0)] = 0; // 未知
        attr_map[std::make_tuple("road_mark", "type", 1)] = 1; // 直行
        attr_map[std::make_tuple("road_mark", "type", 2)] = 3; // 右转
        attr_map[std::make_tuple("road_mark", "type", 3)] = 2; // 左转
        attr_map[std::make_tuple("road_mark", "type", 4)] = 4; // 左转掉头
        attr_map[std::make_tuple("road_mark", "type", 5)] = 5; // 右转掉头
        attr_map[std::make_tuple("road_mark", "type", 6)] = 6; // 向左合流
        attr_map[std::make_tuple("road_mark", "type", 7)] = 7; // 向右合流
        attr_map[std::make_tuple("road_mark", "type", 13)] = 99; // 导流区
    }
}
