 ...
 * @brief ����ģ��
 */
class RoadModelProcMatchLine :
    public fsdmap::process_frame::ProcBase<RoadModelSessionData> {
public:
    RoadModelProcMatchLine() {};
    virtual ~RoadModelProcMatchLine() {};

    const char * name() {
        return "proc_match_line";
    }
...
private:
    ...
    int match_line_center(RoadModelSessionData* session,
         LaneCenterFeature *lc, DisDirPoint &left_lc, DisDirPoint &right_lc);

    LaneLineSample* match_fls_by_lc(RoadModelSessionData* session,
