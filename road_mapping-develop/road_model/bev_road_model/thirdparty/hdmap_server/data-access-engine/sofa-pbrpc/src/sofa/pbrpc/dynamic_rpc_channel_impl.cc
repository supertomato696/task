...
{
public:
    EventHandlerImp(::std::weak_ptr<DynamicRpcChannelImpl> channel) : _channel(channel) {}
    virtual ~EventHandlerImp() {}
    virtual void OnAddressAdded(const std::vector<std::string>& address_list)
    {
        ::std::shared_ptr<DynamicRpcChannelImpl> channel = _channel.lock();
        if (channel) {
...
private:
    ::std::weak_ptr<DynamicRpcChannelImpl> _channel;
};
        ...
        ::google::protobuf::Closure* done)
{
    ...
    while (true) {
        ...
        // sleep to retry
        std::this_thread::sleep_for(std::chrono::microseconds(kRetryInterval * 1000));
        ++retry_count;
    }
