...
public:
    ...
    bool start()
    {
        ...
        for (int i = 0; i < _thread_num; ++i)
        {
            ...
            _threads[i].swap(std::thread(std::bind(ThreadGroupImpl::thread_run, &_thread_params[i])));

            if (!_threads[i].joinable()) {
#if defined( LOG )
                LOG(ERROR) <<  "start(): create thread[" << i << "] failed: error=%d" << -1;
#else
                SLOG(ERROR, "start(): create thread[%d] failed: error=%d", i, -1);
#endif
                _thread_num = i;
                stop();
                return false;
    ...
    {
        dispatch(std::bind(&ThreadGroupImpl::closure_run_helper, handle));
    }

    void post(google::protobuf::Closure* handle)
    {
        post(std::bind(&closure_run_helper, handle));
    }

    void dispatch(ExtClosure<void()>* handle)
    {
        dispatch(std::bind(&ThreadGroupImpl::ext_closure_run_helper, handle));
    }

    void post(ExtClosure<void()>* handle)
    {
        post(std::bind(&ext_closure_run_helper, handle));
    }
