 ...
 * 
 */
#ifndef INTERFACE_RTREE_H_
#define INTERFACE_RTREE_H_
...
///        size allocator for efficiency.
///        Instead of using a callback function for returned results, I
///        recommend and efficient pre-sized, grow-only memory
///        array similar to MFC CArray or STL Vector for returning search query
///        result.
///
...
class RTree {
 ...
 public:
  ...
  // by duans
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
             std::vector<DATATYPE>& vecResult) const;
...
class RTFileStream {
 ...
 public:
  ...
  size_t Write(const TYPE& a_value) {
    ASSERT(m_file);
    return fwrite(reinterpret_cast<void*>(&a_value), sizeof(a_value), 1,
                  m_file);
  }
  ...
  size_t WriteArray(const TYPE* a_array, int a_count) {
    ASSERT(m_file);
    return fwrite(reinterpret_cast<void*>(a_array), sizeof(TYPE) * a_count, 1,
                  m_file);
  }
  ...
  size_t ReadArray(TYPE* a_array, int a_count) {
    ASSERT(m_file);
    return fread(reinterpret_cast<void*>(a_array), sizeof(TYPE) * a_count, 1,
                 m_file);
  }
};
  ...
  if (!RemoveRectRec(a_rect, a_id, *a_root, &reInsertList)) {
    ...
    while (reInsertList) {
      ...
      for (int index = 0; index < tempNode->m_count; ++index) {
        // go over this code. should I use (tempNode->m_level - 1)?
