...
DEFINE_int32(port, 8084, "TCP Port of this server");
...
public:
    ...
    virtual void UploadResource(::google::protobuf::RpcController* controller,
        ...
        brpc::Controller ctl;
        stub.UploadResource(&ctl, request, response, nullptr);
        if (!ctl.Failed()) {
            LOG(INFO) << "UploadResource() response from " << ctl.remote_side()
                      << " to " << ctl.local_side()
            ...
            LOG(WARNING) << "Failed to UploadResource() " << ctl.ErrorText();
    ...
    virtual void QueryResource(::google::protobuf::RpcController* controller,
        ...
        services::TileService_Stub stub(&channel);
        brpc::Controller ctl;
        stub.QueryResource(&ctl, request, response, nullptr);
        if (!ctl.Failed()) {
            LOG(INFO) << "QueryResource() response from " << ctl.remote_side()
                      << " to " << ctl.local_side()
                      << ": " << response->ret().message() << " (code="
                      << response->ret().code() << ")"
                      << " latency=" << ctl.latency_us() << "us";
            ...
            LOG(WARNING) << "Failed to QueryResource() " << ctl.ErrorText();
            cntl->SetFailed(ctl.ErrorText());
        }
    }
    ...
    virtual void DownloadData(::google::protobuf::RpcController* controller,
        ...
        services::TileService_Stub stub(&channel);
        brpc::Controller ctl;
        stub.DownloadData(&ctl, request, response, nullptr);
        if (!ctl.Failed()) {
            LOG(INFO) << "DownloadData() response from " << ctl.remote_side()
                      << " to " << ctl.local_side()
                      << ": " << response->ret().message() << " (code="
                      << response->ret().code() << ")"
                      << " latency=" << ctl.latency_us() << "us";
            ...
            LOG(WARNING) << "Failed to DownloadData() " << ctl.ErrorText();
            cntl->SetFailed(ctl.ErrorText());
        }
    }
};
