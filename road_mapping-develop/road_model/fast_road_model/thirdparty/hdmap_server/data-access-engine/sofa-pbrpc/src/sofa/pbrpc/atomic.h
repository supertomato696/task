...
inline void atomic_inc(volatile T* n) {
    ...
    std::atomic<T>* an = (std::atomic<T>*)n;
    ++(*an);
}
template <typename T>
...
inline T atomic_cmpxchg(volatile T* n, C cmp, D dest) {
    ...
    //return cmp;
    std::atomic<T>* an = (std::atomic<T>*)n;
    an->compare_exchange_strong(cmp, dest);
    return cmp;
}
// return old value
template <typename T>
inline T atomic_swap(volatile T* lockword, T value) {
    ...
    //return value;
    std::atomic<T>* an = (std::atomic<T>*)lockword;
    return an->exchange(value);
}
