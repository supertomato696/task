...
static uint64_t mac48 = get_any_mac48();
struct uuid {
  uint64_t ab;
  uint64_t cd;
...
inline std::string printftime(uint64_t timestamp_secs = 0,
  ...
  try {
    ...
    std::tm tm;
    $msvc(localtime_s(&tm, &t);)
        $melse($windows(tm = *localtime(&t);) $welse(localtime_r(&t, &tm);))
...
inline std::string uuid::pretty() const {
  ...
  version <= 1 &&
      ss << "mac=" << std::setw(4) << (c & 0xFFFF) << std::setw(8) << d << ',';
  version == 4 &&
      ss << "randbits=" << std::setw(8) << (ab & 0xFFFFFFFFFFFF0FFFULL)
         << std::setw(8) << (cd & 0x3FFFFFFFFFFFFFFFULL) << ',';

  ss << std::dec;
  version == 0 && ss << "pid=" << std::setw(4) << (c >> 16) << ',';
  version == 1 &&
      ss << "clock_seq=" << std::setw(4) << ((c >> 16) & 0x3FFF) << ',';
  ...
  uint64_t tv_nsec;
         ...
         inline int gettimeofday(struct timeval *tv, struct timezone *tz) {
           ...
           if (NULL != tv) {
             ...
             // to
             // the 64 bit tmpres, shift it left by 32 then or in the low 32
             // bits.
            ...
            return true;
              ...
              $linux({
                ...
                for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
                  ...
                  /* get an AF_INET* interface address, without lo */
                  if (strcmp(ifa->ifa_name, "lo") && family == AF_INET || family == AF_INET6) {
                ...
                //std::strcpy(ifr.ifr_name, "enp0s31f6");
...
inline uint64_t get_any_mac48() {
  if (mac48 != 0) {
    ...
    if (get_any_mac(node)) {
      ...
      // std::cout << "get_any_mac in get_any_mac48 use time: "
