...
// #include <boost/geometry/strategies/robust/robust_policy.hpp>
...
fsdmap::process_frame::PROC_STATUS RoadModelProcInitData::proc(
    ...
    session->frame_id = session->data_processer->frame_id;
    LOG_INFO("session->frame_id:  {}",session->frame_id);


    std::vector<std::shared_ptr<LinkNodes>> node_list;
    CHECK_FATAL_PROC(session->data_processer->download_nodes(node_list, session->node_id_map), 
            "download_nodes");

    // 从.json文件中读取车道线信息，存储Link关系上的点，点坐标从wgs转换到统一的（0,0,0)坐标系下，在
    // 统一的（0,0,0)坐标系下计算前后点之间的方向向量，以及点之间的前后连接关系，
    // 输出结果 ： raw_links、 raw_links_points
    CHECK_FATAL_PROC(session->data_processer->download_link(session->raw_links, 
                raw_links_points, session->task_polygon, session->task_polygon_vec3d), 
            "download_link");
    ...
    // CHECK_FATAL_PROC(session->data_processer->download_key_pose(raw_key_poses,
    //             false), 
    //         "download_key_pose");
    CHECK_FATAL_PROC(session->data_processer->download_key_pose_mnt(raw_key_poses,
                false), 
            "download_key_pose");
        
    // 可能废弃（里面有红绿灯，但是要从 download_object 读取, TODO:yanxin）
    // CHECK_FATAL_PROC(session->data_processer->download_feature(raw_features), 
    //         "download_feature");
    
    // 读取道路边界
    CHECK_FATAL_PROC(session->data_processer->download_site_boundary_line(raw_boundary_feature),
            "download_site_boundary_line");     

    CHECK_FATAL_PROC(session->data_processer->download_boundary_line(raw_boundary_feature),
            "download_boundary_line");     

    // 读取车道线信息
    CHECK_FATAL_PROC(session->data_processer->download_lane_line(raw_lane_feature),
            "download_lane_line");

    // 读取车道中心信息
    CHECK_FATAL_PROC(session->data_processer->download_lane_center(raw_features),
    ...
    CHECK_FATAL_PROC(session->data_processer->download_object(session->raw_object_ret_list),
            "download_raw_object_ret_list");

    // 读取交叉点识别结果
    CHECK_FATAL_PROC(session->data_processer->download_cross_point(session->split_merge_break_points), "download_cross_point");

    // 读取路口边界以及不可通行区域
    // CHECK_FATAL_PROC(session->data_processer->download_junction(session->new_junction_list, session->new_area_list),
    //         "download_raw_object_junction_list");

    CHECK_FATAL_PROC(update_global_line_id(session), "update_global_line_id");
    ...
    CHECK_FATAL_PROC(init_intersections(session), "init_intersections");
...
int RoadModelProcInitData::update_global_line_id(RoadModelSessionData* session) {
    session->lane_center_id = session->data_processer->lane_center_id;
    session->lane_boundary_id = session->data_processer->lane_boundary_id;
    session->road_boundary_id = session->data_processer->road_boundary_id;
    LOG_INFO("lane_center_id:{}, lane_boundary_id:{}, road_boundary_id:{}",
            session->lane_center_id, session->lane_boundary_id, session->road_boundary_id);
...
int RoadModelProcInitData::init_intersections(RoadModelSessionData* session) {
...
int RoadModelProcInitData::init_trail(RoadModelSessionData* session) {
    ...
    for (auto &key_pose : raw_key_poses) { // 把raw_key_poses的结果放入key_pose_map
...
int RoadModelProcInitData::init_feature_line(RoadModelSessionData* session) {
    for (auto &feature : raw_features) {
        ...
        if (ele_type == ELEMENT_LANE_CENTER) {
...
int RoadModelProcInitData::save_debug_info(RoadModelSessionData* session) {
    ...
    for (auto &trail : session->key_pose_map) {
        ...
        for (auto &key_pose : trail_ptr->second.list) {
            ...
            ele.label.intensity_opt = key_pose->raw_no;
    ...
    for (auto &link : session->raw_links) {
        ...
        for (auto &pt : link->list) {
            ...
            int random_number = dis(gen);
            // ele.label.intensity_opt=random_number;
            ele.label.intensity_opt=std::stod(link->id);
            ele.label.score=alg::calc_theta(pt->dir);
        }

        // std::cout << "i : " << i << " link->id: " << link->id << std::endl;

        // // 打印 link id
        // Eigen::Vector3d mid;
        // mid.setZero();
        // if(link->list.size() > 2) {
        //     mid += link->list.front()->pos;
        //     mid += link->list.back()->pos;
        // }
        // mid /= 2.0;
        // // link_id
        // {
        //     // double curvature = std::round(split_merge_point[k]->curvature * 1000)/1000;
        //     auto log_link_id= session->add_debug_log(utils::DisplayInfo::TEXT, "{}", link->id);
        //     log_link_id->color = {255, 0, 128}; 
        //     log_link_id->add(mid);
        // }
    ...
    session->set_display_name("raw_data_sem");
    for (auto &it : session->lane_center_instance_map) {
        auto ins_ptr = &it.second;
        auto log = session->add_debug_log(utils::DisplayInfo::POINT, "raw_center_line[id={}]", it.first);
        auto line_log = session->add_debug_log(utils::DisplayInfo::LINE, "raw_center_line[id={}]", it.first);
        session->thread_pool->schedule(
                [log, line_log,ins_ptr, session, this](utils::ProcessBar *process_bar) {
        auto &first_feature = ins_ptr->list.front();
        log->color = {0, 255, 0};
        for (auto &feature : ins_ptr->list) {
            line_log->color= {0, 255, 0};
            line_log->add(feature->pos);
            auto &ele = log->add(feature->pos, -1, feature->type);
            ele.label.score = feature->score;
            ele.label.cloud_pano_seg=feature->type;
            ele.label.cloud_line_seg=feature->color;
            if (feature->boundary_type == LaneType::RIGHT_TURN_LB_LEFT or feature->boundary_type == LaneType::RIGHT_TURN_LB_RIGHT) {
                line_log->color = {255, 0, 0}; // 红色
            }
        }
        });
    }
    session->thread_pool->wait(1, "raw_data_center_line_sem");
    // session->save_debug_info("raw_data_center_line_sem");
    ...
    for (auto &it : session->sem_lane_instance_map) {
        ...
        auto line_log = session->add_debug_log(utils::DisplayInfo::LINE, "raw_lane_line[id={}]", it.first);
        auto ins_ptr = &it.second;
        session->thread_pool->schedule(
                [log,line_log, ins_ptr, session, this](utils::ProcessBar *process_bar) {
        auto &first_feature = ins_ptr->list.front();
        ...
        for (auto &feature : ins_ptr->list) {
            line_log->color={255, 255, 255};
            auto &ele = log->add(feature->pos, -1, feature->type);
            auto &lele=line_log->add(feature->pos, -1, feature->type);
            ele.label.score = feature->score;
            switch(feature->type)
            {
               case 2:
               {
                ...
                lele.color={255, 255, 255}; //白色
                break;
               }
               case 3:
               {
                ele.color={255, 140, 0}; //黄色
                lele.color={255, 140, 0}; //黄色
                break;
               }
               default:
               {
                ele.color={0, 255, 0};
                lele.color={0, 255, 0}; //黄色
               }

            }

            if (feature->boundary_type == LaneType::RIGHT_TURN_LC) {
                line_log->color={0, 255, 255};
            }
    ...
    for (auto &obj : session->raw_object_ret_list) {
        ...
        log->add( obj->list.front()->pos, 1, obj->ele_type);
    }
    // for (auto &obj : session->new_junction_list) {
    //     auto log = session->add_debug_log(utils::DisplayInfo::LINE, "raw_junction");
    //     log->color = {18, 232, 228};//青色
    //     for (auto &pt : obj->point_info) {
    //         auto &ele = log->add(pt->pos, 1, obj->ele_type);
    //     }
    //     log->add( obj->point_info.front()->pos, 1, obj->ele_type);
    // }
    for (auto &obj : session->raw_intersections) {
        auto log = session->add_debug_log(utils::DisplayInfo::LINE_INDEX, "raw_intersection");
        log->color = {18, 232, 228};//青色
        for (auto &pt : obj->point_info) {
            auto &ele = log->add(pt->pos, 1, obj->ele_type);  
        }
    ...
    session->save_debug_info("raw_data_sem");
