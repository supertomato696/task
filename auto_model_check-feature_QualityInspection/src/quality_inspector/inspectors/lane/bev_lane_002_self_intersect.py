...
@BaseInspector.register('车道面自相交检测')
class InspectSelfIntersectLane(BasePolygonInespector):

    def __init__(self, sd_link, intersection, data_dir):
        super().__init__(sd_link, intersection, data_dir)
        # 不用特意拍平
        self.polygons_gdf = self.check_polygon_geom_type(self.intersection.lanes)
    ...
    def inspect(self):
        ...
        if self.total_count == 0:
            logger.warning('当前路口没有车道面，不需要检查，BYE！')
            return
        ...
        self.result['自相交车道面'] = self_intersect_poly
