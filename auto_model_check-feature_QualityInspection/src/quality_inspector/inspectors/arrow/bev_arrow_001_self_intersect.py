...
@BaseInspector.register('地面箭头自相交检测')
class InspectSelfIntersectArrow(BasePolygonInespector):

    def __init__(self, sd_link, intersection, data_dir):
        super().__init__(sd_link, intersection, data_dir)
        # 不用特意拍平
        self.polygons_gdf = self.check_polygon_geom_type(self.intersection.arrows)
    ...
    def inspect(self):
        ...
        if self.total_count == 0:
            logger.warning('当前路口没有地面箭头，不需要检查，BYE！')
            return
        ...
        self.result['自相交地面箭头'] = self_intersect_poly
