...
@BaseInspector.register('道路边界自相交检测')
class InspectSelfIntersectBoundary(BaseLineInespector):
    
    def __init__(self, sd_link, intersection, data_dir):
        super().__init__(sd_link, intersection, data_dir)
        self.boundaries = ensure_single_geometries_v2(self.check_line_geom_type(self.intersection.road_boundaries))
        self.total_count = self.boundaries['ID'].nunique() if not self.boundaries.empty else 0
        self.log_count = 0
    ...
    def inspect(self):
        ...
        # 保存结果
        self.result['自相交道路边界'] = intersect_lines
