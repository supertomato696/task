...
@BaseInspector.register('道路面自相交检测')
class InspectSelfIntersectRoad(BasePolygonInespector):

    def __init__(self, sd_link, intersection, data_dir):
        super().__init__(sd_link, intersection, data_dir)
        # 不用特意拍平
        self.polygons_gdf = self.check_polygon_geom_type(self.intersection.roads)
    ...
    def inspect(self):
        ...
        self.result['自相交道路面'] = self_intersect_poly
