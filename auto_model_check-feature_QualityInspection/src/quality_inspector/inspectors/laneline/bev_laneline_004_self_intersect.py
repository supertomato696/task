...
@BaseInspector.register('车道线自相交检测')
class InspectSelfIntersectLaneLine(BaseLineInespector):

    def __init__(self, sd_link, intersection, data_dir):
        super().__init__(sd_link, intersection, data_dir)
        self.lanelines_gdf = ensure_single_geometries_v2(self.check_line_geom_type(self.intersection.lane_lines))
    ...
    def inspect(self):
        ...
        logger.info(f"车道线自相交数量: {self.log_count}, 车道线总数: {self.total_count}")
        # 保存结果
        self.result['自相交车道线'] = intersect_lines
