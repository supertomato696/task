...
@BaseInspector.register('停止线交叉检测')
class DetectIntersectStopLine(BaseLineInespector):
    
    def __init__(self, sd_link, intersection, data_dir):
        super().__init__(sd_link, intersection, data_dir)
        self.stoplines_gdf = self.check_and_single_line_geom(self.intersection.stop_lines)
        self.total_count = self.stoplines_gdf['ID'].nunique() if not self.stoplines_gdf.empty else 0
        self.log_count = 0
    ...
    def inspect(self):
        ...
        lines, intersect_pts = LineProcessor.check_line_intersection(self.stoplines_gdf)
