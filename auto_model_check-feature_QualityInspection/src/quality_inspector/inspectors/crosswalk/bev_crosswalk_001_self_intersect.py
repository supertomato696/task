...
@BaseInspector.register('人行横道自相交检测')
class InspectIntersectCrosswalk(BasePolygonInespector):
    ...
    def __init__(self, sd_link, intersection, data_dir):
        ...
        # 不用特意拍平
        self.polygons_gdf = self.check_polygon_geom_type(self.intersection.crosswalks)
    ...
    def inspect(self):
        ...
        if self.total_count == 0:
            logger.warning('当前路口没有人行横道，不需要检查，BYE！')
            return
        ...
        self.result['自相交人行横道'] = self_intersect_poly
